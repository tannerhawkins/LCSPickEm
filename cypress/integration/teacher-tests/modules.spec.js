describe("Modules", () => {
  ...
  it("allows teacher create new module", () => {
    ...
    cy.get("[data-test=step-container]").children().first().should("contain", "text")
    ...
    cy.get("[data-test=video]").type("https://www.youtube.com/watch?v=sKrsE2lBhu4&ab_channel=Coldplay")
    cy.get("[data-test=submit]").click()
    ...
    cy.get("[data-test=step-container]").children().eq(1).should("contain", "video")
    ...
    cy.get("[data-test=module-title]").last().should("contain", "Test Module 3")
  });
