(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[16],{156:function(A,i,e){"use strict";e.r(i);var t=e(2),a=e(36),o=e(72),c=e(71),s=e(66),g=e(25),n=e(67),E=e(11),C=e(19),B=e(84),d=e(16),l=e(38),r=e(21),I=e(23),h=e(5);const Q=s.a.div`
  margin-top: ${g.a.HEADER_HEIGHT};
  margin-left: ${g.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,x=s.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,u=s.a.p`
  font-size: 20px;
`,R=s.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,m=s.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,p=s.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,j=Object(s.a)(n.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${g.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,b=Object(s.a)(n.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${g.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,O=s.a.p`
  color: red;
`,J=s.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;i.default=()=>{const A=Object(E.g)(),[i,e]=Object(t.useState)(),s=Object(d.c)(l.a);Object(d.c)(r.b);return Object(h.jsxs)(a.a,{title:"Add a Student",description:"Add a Student",children:[Object(h.jsx)(o.a,{}),Object(h.jsx)(c.a,{}),Object(h.jsxs)(Q,{children:[Object(h.jsx)(x,{children:"Add Students to a Class"}),Object(h.jsx)(u,{children:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce ac purus eget lacus pellentesque consequat. Donec commodo tincidunt lorem, c ursus accumsan eros varius vel."}),Object(h.jsxs)(m,{id:"studentsForm",children:[Object(h.jsx)(J,{children:"Student's Email"}),Object(h.jsx)(p,{type:"text",name:"email","data-test":"email",required:!0}),Object(h.jsx)(J,{children:"Add to Class"}),Object(h.jsx)(B.a,{}),Object(h.jsx)(O,{"data-test":"error",children:i}),Object(h.jsxs)(R,{children:[Object(h.jsx)(j,{onClick:()=>{e();const A=Object.values(document.forms.studentsForm).reduce(((A,i)=>(A[i.name]=i.value,A)),{});""!==A.email&&A.email.includes("@")?C.e.where("email","==",A.email).get().then((A=>{var i;const t=null===(i=A.docs[0])||void 0===i?void 0:i.data();void 0!==t&&"student"===t.accountType?t.classList.map((A=>A.cid)).includes(s.cid)?e("Student already in class"):(C.c.doc(s.cid).update({students:I.a.firestore.FieldValue.arrayUnion({uid:t.uid,displayName:t.displayName,email:t.email})}),C.e.doc(t.uid).update({classList:I.a.firestore.FieldValue.arrayUnion({cid:s.cid,modules:s.modules,className:s.className})}),e(`Added ${t.displayName} to ${s.className}`),document.getElementById("studentsForm").reset()):e("Student not found")})):e("Please enter a valid email")},"data-test":"submit",children:"ADD TO CLASS"}),Object(h.jsx)(b,{onClick:()=>A.push("teacher/home"),"data-test":"back",children:"BACK TO DASHBOARD"})]})]})]})]})}},67:function(A,i,e){"use strict";var t=e(25);const a=e(66).a.div`
  background-color: ${t.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=a},69:function(A,i,e){"use strict";i.a=e.p+"static/media/white mtn.bb2904a1.png"},71:function(A,i,e){"use strict";e(2);var t=e(66),a=e(25),o=e(75),c=e(5);const s=t.a.img`
  height: 50%;
  border-radius: 50%;
`;var g=()=>Object(c.jsx)(s,{src:"data:image/png;base64,"}),n=e(76),E=e(16),C=e(11),B=e(21),d=e(77);const l=t.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,r=Object(t.a)(d.a)`
  height: 50%;
`,I=t.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 800px) {
    display: none;
  }
`,h=t.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,Q=t.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var x=({className:A})=>{const i=Object(E.c)(B.c),e=Object(E.c)(B.a),t=Object(C.g)();return Object(c.jsxs)(l,{className:A,onClick:()=>t.push("/profile"),children:[Object(c.jsx)(r,{}),Object(c.jsxs)(I,{children:[Object(c.jsx)(h,{"data-test":"display-name",children:i}),Object(c.jsx)(Q,{"data-test":"account-type",children:e})]})]})},u=(e(37),e(22));const R=t.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${a.a.HEADER_HEIGHT};
  left: ${a.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${a.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,m=t.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,p=t.a.div`
  width: 50%;
  height: 100%;
`,j=t.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,b=t.a.img`
  height: 45%;
`,O=t.a.img`
  height: 40%;
`,J=t.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,Y=Object(t.a)(x)``;i.a=()=>{const A=Object(E.b)();return Object(c.jsxs)(R,{id:"header",children:[Object(c.jsx)(p,{}),Object(c.jsxs)(m,{children:[Object(c.jsx)(Y,{}),Object(c.jsxs)(j,{children:[Object(c.jsx)(b,{src:o.a}),Object(c.jsx)(O,{src:"data:image/png;base64,"})]}),Object(c.jsx)(g,{}),Object(c.jsx)(J,{onClick:()=>A(Object(u.a)()),src:n.a})]})]})}},72:function(A,i,e){"use strict";e(2);var t=e(66),a=e(25),o=e(69),c=e(11),s=e(5);const g=t.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${a.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${a.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,n=t.a.div`
  background-color: ${a.a.COLOR.GREEN};
  height: ${a.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,E=t.a.img`
  width: 60px;
`,C=t.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(c.g)();return Object(s.jsxs)(g,{children:[Object(s.jsx)(n,{children:Object(s.jsx)(E,{onClick:()=>A.push("/home"),src:o.a})}),Object(s.jsx)(C,{src:"data:image/png;base64,"}),Object(s.jsx)(C,{src:"data:image/png;base64,"}),Object(s.jsx)(C,{src:"data:image/png;base64,"})]})}},75:function(A,i,e){"use strict";i.a="data:image/png;base64,"},76:function(A,i,e){"use strict";i.a="data:image/png;base64,"},77:function(A,i,e){"use strict";e(2);var t=e(66),a=e.p+"static/media/default profile icon.591db299.png",o=e(5);const c=t.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(o.jsx)(c,{className:A,src:a})},84:function(A,i,e){"use strict";var t=e(2),a=e(16),o=e(66),c=e(21),s=e(30),g=e(38),n=e(25),E=e(19),C=e(5);const B=o.a.form`
  display: flex;
  flex-flow: column;
  justify-content: flex-start;
`,d=o.a.select`
  height: 100%;
  padding: 0.5rem;
  border: 0px;
  outline: 0px;
  font-size: 25px;
  color: ${n.a.COLOR.GREEN};
  background-color: ${n.a.COLOR.GRAY};
`,l=o.a.option`
  color: ${A=>A.defaultValue?"lightgrey":"black"};
`;i.a=A=>{const i=Object(a.b)(),e=Object(a.c)(c.b),o=Object(a.c)(g.a);Object(t.useEffect)((()=>{}));return e[0]?Object(C.jsx)(B,{action:A.action,"data-test":"dropdown",children:Object(C.jsx)(d,{id:"services","data-test":"class",name:"services",onChange:A=>{const e=Array.from(A.target.children).filter((A=>A.selected)).map((A=>A.dataset.cid))[0];E.c.doc(e).get().then((A=>i(Object(s.a)(A.data()))))},defaultValue:null===o||void 0===o?void 0:o.cid,children:e.map((A=>Object(C.jsx)(l,{value:A.cid,"data-cid":A.cid,classItem:A,children:A.className},A.cid)))})}):Object(C.jsx)(C.Fragment,{})}}}]);
//# sourceMappingURL=16.46bae635.chunk.js.map