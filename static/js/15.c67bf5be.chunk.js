(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[15],{233:function(A,i,e){"use strict";e.r(i);var t=e(2),o=e(36),a=e(72),c=e(73),g=e(66),C=e(25),s=e(67),E=e(11),B=e(19),n=e(16),I=e(21),h=e(23),d=e(37),Q=e(30),l=e(5);const r=g.a.div`
  margin-top: ${C.a.HEADER_HEIGHT};
  margin-left: ${C.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,x=g.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,R=g.a.p`
  font-size: 20px;
`,p=g.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,J=g.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,j=g.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,Y=Object(g.a)(s.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${C.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,u=Object(g.a)(s.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${C.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,m=g.a.p`
  color: red;
`,b=g.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;i.default=()=>{const A=Object(E.g)(),[i,e]=Object(t.useState)(),g=Object(n.b)(),C=Object(n.c)(I.j);return Object(l.jsxs)(o.a,{title:"Add a Class",description:"Add a Class",children:[Object(l.jsx)(a.a,{}),Object(l.jsx)(c.a,{}),Object(l.jsxs)(r,{children:[Object(l.jsx)(x,{children:"Add a Class"}),Object(l.jsx)(R,{children:"Please enter the enrollment code of the class you would like to enroll in."}),Object(l.jsxs)(J,{id:"enrollmentForm",children:[Object(l.jsx)(b,{children:"Enrollment Code"}),Object(l.jsx)(j,{type:"text",name:"code","data-test":"enrollment-code",required:!0}),Object(l.jsx)(m,{children:i}),Object(l.jsxs)(p,{children:[Object(l.jsx)(Y,{onClick:()=>{e();const i=Object.values(document.forms.enrollmentForm).reduce(((A,i)=>(A[i.name]=i.value,A)),{});""!==i.code?B.c.where("cid","==",i.code).get().then((i=>{var t;const o=null===(t=i.docs[0])||void 0===t?void 0:t.data();void 0!==o?o.students.map((A=>A.uid)).includes(C.uid)?e("Student already in class"):(B.c.doc(o.cid).update({students:h.a.firestore.FieldValue.arrayUnion({uid:C.uid,displayName:C.displayName,email:C.email})}),B.e.doc(C.uid).update({classList:h.a.firestore.FieldValue.arrayUnion({cid:o.cid,modules:o.modules,className:o.className})}).then((()=>{e(`Added ${C.displayName} to ${o.className}`),B.e.doc(C.uid).get().then((A=>g(Object(d.b)(A.data())))),B.c.doc(o.cid).get().then((i=>{g(Object(Q.a)(i.data())),A.push("/student/home")}))}))):e("Class not found")})):e("Please enter a valid code")},"data-test":"submit",children:"ADD CLASS"}),Object(l.jsx)(u,{onClick:()=>A.push("student/home"),children:"BACK TO DASHBOARD"})]})]})]})]})}},67:function(A,i,e){"use strict";var t=e(25);const o=e(66).a.div`
  background-color: ${t.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=o},72:function(A,i,e){"use strict";e(2);var t=e(66),o=e(25),a=e(74),c=e(11),g=e(5);const C=t.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${o.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${o.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,s=t.a.div`
  background-color: ${o.a.COLOR.GREEN};
  height: ${o.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,E=t.a.img`
  width: 60px;
`,B=t.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(c.g)();return Object(g.jsxs)(C,{children:[Object(g.jsx)(s,{children:Object(g.jsx)(E,{onClick:()=>A.push("/home"),src:a.a})}),Object(g.jsx)(B,{src:"data:image/png;base64,"}),Object(g.jsx)(B,{src:"data:image/png;base64,"}),Object(g.jsx)(B,{src:"data:image/png;base64,"})]})}},73:function(A,i,e){"use strict";e(2);var t=e(66),o=e(25),a=e(75),c=e(77),g=e(5);const C=t.a.img`
  height: 50%;
  border-radius: 50%;
`;var s=()=>Object(g.jsx)(C,{src:"data:image/png;base64,"}),E=e(78),B=e(79),n=e(16),I=(e(37),e(22));const h=t.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${o.a.HEADER_HEIGHT};
  left: ${o.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${o.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,d=t.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,Q=t.a.div`
  width: 50%;
  height: 100%;
`,l=t.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,r=t.a.img`
  height: 45%;
`,x=t.a.img`
  height: 40%;
`,R=t.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,p=Object(t.a)(B.a)``;i.a=()=>{const A=Object(n.b)();return Object(g.jsxs)(h,{id:"header",children:[Object(g.jsx)(Q,{}),Object(g.jsxs)(d,{children:[Object(g.jsx)(p,{}),Object(g.jsxs)(l,{children:[Object(g.jsx)(r,{src:c.a}),Object(g.jsx)(x,{src:a.a})]}),Object(g.jsx)(s,{}),Object(g.jsx)(R,{onClick:()=>A(Object(I.a)()),src:E.a})]})]})}},74:function(A,i,e){"use strict";i.a=e.p+"static/media/white mtn.bb2904a1.png"},75:function(A,i,e){"use strict";i.a="data:image/png;base64,"},77:function(A,i,e){"use strict";i.a="data:image/png;base64,"},78:function(A,i,e){"use strict";i.a="data:image/png;base64,"},79:function(A,i,e){"use strict";e(2);var t=e(16),o=e(11),a=e(66),c=e(21),g=(e(75),e(80)),C=e(5);const s=a.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,E=Object(a.a)(g.a)`
  height: 50%;
`,B=a.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 400px) {
    display: none;
  }
`,n=a.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,I=a.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;i.a=({className:A})=>{const i=Object(t.c)(c.c),e=Object(t.c)(c.a),a=Object(o.g)();return Object(C.jsxs)(s,{className:A,onClick:()=>a.push("/profile"),children:[Object(C.jsx)(E,{}),Object(C.jsxs)(B,{children:[Object(C.jsx)(n,{"data-test":"display-name",children:i}),Object(C.jsx)(I,{"data-test":"account-type",children:e})]})]})}},80:function(A,i,e){"use strict";e(2);var t=e(66),o=e.p+"static/media/default profile icon.591db299.png",a=e(5);const c=t.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(a.jsx)(c,{className:A,src:o})}}]);
//# sourceMappingURL=15.c67bf5be.chunk.js.map