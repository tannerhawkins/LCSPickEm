(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[9],{49:function(A,i,e){"use strict";e.d(i,"a",(function(){return t}));const t={COLOR:{LIGHT_GREEN:"#A2E1D0",DARK_GREEN:"#6BAE9B",GREEN:"#7DCBB5",WHITE:"#e9e9ff",DARK_BLUE:"#373B53",TEAL:"#B5FFEA",GRAY:"#E5E5E5"},HEADER_HEIGHT:"80px",SIDEBAR_WIDTH:"80px",EMPTY_MODULE:{title:"",description:"",public:!0,creator:void 0,steps:[]},MOCK_MODULE:{title:"Mock Module",description:"This module is used for testing and development",public:!0,creator:"someteacherIDhere",steps:[{type:"text",data:{}},{type:"video",data:{}},{type:"quiz",data:{}}]}}},50:function(A,i,e){"use strict";e.d(i,"d",(function(){return c})),e.d(i,"b",(function(){return n})),e.d(i,"c",(function(){return s})),e.d(i,"a",(function(){return a}));var t=e(60);e(65);t.a.initializeApp({apiKey:"AIzaSyBikWf9vy86sF9wXyRjkpdrrAJaw7WoQ8M",authDomain:"ancient-path-adventures.firebaseapp.com",projectId:"ancient-path-adventures",storageBucket:"ancient-path-adventures.appspot.com",messagingSenderId:"1051282870332",appId:"1:1051282870332:web:8f7a5922db7d370e86b339",measurementId:"G-NPPQJQTEQN"});const o=t.a.firestore();var a=t.a.auth();new t.a.auth.GoogleAuthProvider;const c=o.collection("userData"),n=o.collection("classData"),s=o.collection("modules")},51:function(A,i,e){"use strict";var t=e(49);const o=e(48).a.div`
  background-color: ${t.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=o},52:function(A,i,e){"use strict";i.a=e.p+"static/media/white mtn.bb2904a1.png"},53:function(A,i,e){"use strict";e.d(i,"a",(function(){return t}));e(12),e(50);const t=A=>({type:"SIGN_IN",payload:A})},54:function(A,i,e){"use strict";i.a="data:image/png;base64,"},55:function(A,i,e){"use strict";i.a="data:image/png;base64,"},56:function(A,i,e){"use strict";e(0);var t=e(48),o=e(49),a="data:image/png;base64,",c=e(54),n=e(2);const s=t.a.img`
  height: 50%;
  border-radius: 50%;
`;var E=()=>Object(n.jsx)(s,{src:"data:image/png;base64,"}),g=e(55),C=e(12),B=e(14),d=e(58);const r=t.a.div`
  height: 100%;
  display: flex;
  align-items: center;
`,I=t.a.img`
  height: 40%;
`,h=Object(t.a)(d.a)`
  height: 50%;
`,l=t.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 10px;

  @media (max-width: 800px) {
    display: none;
  }
`,Q=t.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,x=t.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var p=({className:A})=>{const i=Object(C.c)(B.c),e=Object(C.c)(B.a);return Object(n.jsxs)(r,{className:A,children:[Object(n.jsx)(h,{}),Object(n.jsx)(I,{src:a}),Object(n.jsxs)(l,{children:[Object(n.jsx)(Q,{children:i}),Object(n.jsx)(x,{children:e})]})]})},u=(e(53),e(11));const R=t.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${o.a.HEADER_HEIGHT};
  left: ${o.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${o.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,J=t.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,m=t.a.div`
  width: 50%;
  height: 100%;
`,b=t.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,j=t.a.img`
  height: 45%;
`,Y=t.a.img`
  height: 40%;
`,U=t.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,O=Object(t.a)(p)``;i.a=()=>{const A=Object(C.b)();return Object(n.jsxs)(R,{id:"header",children:[Object(n.jsx)(m,{}),Object(n.jsxs)(J,{children:[Object(n.jsx)(O,{}),Object(n.jsxs)(b,{children:[Object(n.jsx)(j,{src:c.a}),Object(n.jsx)(Y,{src:a})]}),Object(n.jsx)(E,{}),Object(n.jsx)(U,{onClick:()=>A(Object(u.a)()),src:g.a})]})]})}},57:function(A,i,e){"use strict";e(0);var t=e(48),o=e(49),a=e(52),c=e(5),n=e(2);const s=t.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${o.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${o.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,E=t.a.div`
  background-color: ${o.a.COLOR.GREEN};
  height: ${o.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,g=t.a.img`
  width: 60px;
`,C=t.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(c.g)();return Object(n.jsxs)(s,{children:[Object(n.jsx)(E,{children:Object(n.jsx)(g,{onClick:()=>A.push("/home"),src:a.a})}),Object(n.jsx)(C,{src:"data:image/png;base64,"}),Object(n.jsx)(C,{src:"data:image/png;base64,"}),Object(n.jsx)(C,{src:"data:image/png;base64,"})]})}},58:function(A,i,e){"use strict";e(0);var t=e(48),o=e.p+"static/media/default profile icon.591db299.png",a=e(2);const c=t.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(a.jsx)(c,{className:A,src:o})},62:function(A,i,e){"use strict";e.d(i,"a",(function(){return t}));e(50);const t=A=>({type:"SET_CURRENT_CLASS",payload:A})},90:function(A,i,e){"use strict";e.r(i);var t=e(0),o=e(23),a=e(57),c=e(56),n=e(48),s=e(49),E=e(51),g=e(5),C=e(50),B=e(12),d=e(14),r=e(60),I=e(53),h=e(62),l=e(2);const Q=n.a.div`
  margin-top: ${s.a.HEADER_HEIGHT};
  margin-left: ${s.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,x=n.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,p=n.a.p`
  font-size: 20px;
`,u=n.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,R=n.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,J=n.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,m=Object(n.a)(E.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,b=Object(n.a)(E.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,j=n.a.p`
  color: red;
`,Y=n.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;i.default=()=>{const A=Object(g.g)(),[i,e]=Object(t.useState)(),n=Object(B.b)(),s=Object(B.c)(d.h);return Object(l.jsxs)(o.a,{title:"Add a Class",description:"Add a Class",children:[Object(l.jsx)(a.a,{}),Object(l.jsx)(c.a,{}),Object(l.jsxs)(Q,{children:[Object(l.jsx)(x,{children:"Add a Class"}),Object(l.jsx)(p,{children:"Please enter the enrollment code of the class you would like to enroll in."}),Object(l.jsxs)(R,{id:"enrollmentForm",children:[Object(l.jsx)(Y,{children:"Enrollment Code"}),Object(l.jsx)(J,{type:"text",name:"code",required:!0}),Object(l.jsx)(j,{children:i}),Object(l.jsxs)(u,{children:[Object(l.jsx)(m,{onClick:()=>{e();const i=Object.values(document.forms.enrollmentForm).reduce(((A,i)=>(A[i.name]=i.value,A)),{});""!==i.code?C.b.where("cid","==",i.code).get().then((i=>{var t;const o=null===(t=i.docs[0])||void 0===t?void 0:t.data();void 0!==o?o.students.map((A=>A.uid)).includes(s.uid)?e("Student already in class"):(C.b.doc(o.cid).update({students:r.a.firestore.FieldValue.arrayUnion({uid:s.uid,displayName:s.displayName,email:s.email})}),C.d.doc(s.uid).update({classList:r.a.firestore.FieldValue.arrayUnion({cid:o.cid,modules:o.modules,className:o.className})}).then((()=>{e(`Added ${s.displayName} to ${o.className}`),C.d.doc(s.uid).get().then((A=>n(Object(I.a)(A.data())))),C.b.doc(o.cid).get().then((i=>{n(Object(h.a)(i.data())),A.push("/student/home")}))}))):e("Class not found")})):e("Please enter a valid code")},children:"ADD CLASS"}),Object(l.jsx)(b,{onClick:()=>A.push("student/home"),children:"BACK TO DASHBOARD"})]})]})]})]})}}}]);
//# sourceMappingURL=9.9362a082.chunk.js.map