(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[3],{141:function(A,i,e){"use strict";i.a=e.p+"static/media/text icon.b8202f2f.png"},142:function(A,i,e){"use strict";i.a=e.p+"static/media/quiz icon.0ea0620a.png"},143:function(A,i,e){"use strict";i.a="data:image/png;base64,"},144:function(A,i,e){"use strict";i.a=e.p+"static/media/ebook icon.7e0b328b.png"},149:function(A,i,e){"use strict";var t=e(2),a=e(16),c=e(11),o=e(66),g=e(68),E=e(70),n=e(25),C=e(141),s=e(142),I=e(143),B=e(144),r=e(5);const Q=o.a.div`
  background-color: ${n.a.COLOR.GREEN};
  border-radius: 4px;
  width: 70%;
  height: 90px;
  margin: 8px 0px;
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 10px 30px;
`,d=o.a.p`
  text-align: center;
  font-size: 30px;
  color: white;
`,h=o.a.img`
  height: 90%;
`;var l=({id:A,type:i,handleDrag:e,handleDrop:t,onClick:a})=>{const c=()=>{switch(i){case"text":return Object(r.jsx)(h,{src:C.a});case"quiz":return Object(r.jsx)(h,{src:s.a});case"ebook":return Object(r.jsx)(h,{src:B.a});case"video":return Object(r.jsx)(h,{src:I.a});default:return Object(r.jsx)(r.Fragment,{})}};return Object(r.jsxs)(Q,{draggable:!0,id:A,onDragOver:A=>A.preventDefault(),onDragStart:e,onDrop:t,onClick:a,children:[Object(r.jsx)(c,{}),Object(r.jsx)(d,{children:i.toUpperCase()})]})};const p=o.a.div`
  display: flex;
  flex-direction: column;
  margin: 40px 0px;
`;i.a=A=>{const i=Object(a.b)(),e=Object(c.g)(),[o,n]=Object(t.useState)(),C=Object(a.c)(E.h),s=A=>{n(A.currentTarget.id)},I=A=>{const e=C.find((A=>A.id===parseInt(o))),t=C.find((i=>i.id===parseInt(A.currentTarget.id))),a=e.order,c=t.order,E=C.map((i=>(i.id===parseInt(o)&&(i.order=c),i.id===parseInt(A.currentTarget.id)&&(i.order=a),i)));i(Object(g.f)(E))};return Object(r.jsx)(p,{"data-test":"step-container",children:C.sort(((A,i)=>A.order-i.order)).map((A=>Object(r.jsx)(l,{id:parseInt(A.id),boxColor:A.color,type:A.type,handleDrag:s,handleDrop:I,onClick:()=>{return t=A,i(Object(g.e)(t)),void e.push(`/teacher/create-module/edit-${t.type}`);var t},"data-test":"box"},parseInt(A.id))))})}},150:function(A,i,e){"use strict";e(2);var t=e(16),a=e(66),c=e(25),o=e(67),g=e(5);const E=a.a.div`
  background-color: white;
  border-radius: 4px;
  width: 40%;
  margin: 8px 0px;
  display: flex;
  align-items: center;
  position: relative;
  justify-content: center;
  padding: 30px;
`,n=a.a.p`
  top: 10px;
  right: 20px;
  position: absolute;
  :hover {
    cursor: pointer;
  }
`,C=a.a.div`
  height: 100vh;
  width: 100vw;
  display: flex;
  position: fixed;
  z-index: 10000;
  align-items: center;
  justify-content: center;
  top: 0;
  background: rgba(0, 0, 0, 0.6);
`,s=a.a.div`
  width: 100%;
`;var I=A=>Object(g.jsx)(C,{style:A.style,children:Object(g.jsxs)(E,{className:A.className,children:[Object(g.jsx)(n,{onClick:A.onClose,children:"X"}),Object(g.jsx)(s,{children:A.children})]})}),B=e(70),r=e(11),Q=e(141),d=e(142),h=e(143),l=e(144),p=e(21),x=e(68);const b=Object(a.a)(I)``,u=a.a.p`
  text-align: center;
  font-size: 30px;
`,R=a.a.img`
  height: 90%;
`,j=a.a.div`
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: space-between;
  width: 100%;
`,J=Object(a.a)(o.a)`
  background-color: ${c.a.COLOR.GREEN};
  border-radius: 4px;
  width: 50%;
  height: 90px;
  margin: 8px 0px;
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 10px 30px;
  padding-right: 10%;
  font-size: 50px;
`;i.a=({className:A,onClose:i,style:e})=>{const a=Object(r.g)(),c=Object(t.b)(),o=(Object(t.c)(B.h),Object(t.c)(p.a)),E=A=>{c(Object(x.e)(void 0)),a.push(`/${o}/create-module/edit-${A}`)};return Object(g.jsxs)(b,{style:e,className:A,onClose:i,children:[Object(g.jsx)(u,{children:"Select Step to Add"}),Object(g.jsxs)(j,{children:[Object(g.jsxs)(J,{onClick:()=>E("text"),"data-test":"add-text",children:[Object(g.jsx)(R,{src:Q.a}),"Text"]}),Object(g.jsxs)(J,{onClick:()=>E("video"),"data-test":"add-video",children:[Object(g.jsx)(R,{src:h.a}),"Video"]}),Object(g.jsxs)(J,{onClick:()=>E("quiz"),"data-test":"add-quiz",children:[Object(g.jsx)(R,{src:d.a}),"Quiz"]}),"admin"===o&&Object(g.jsxs)(J,{onClick:()=>E("ebook"),"data-test":"add-ebook",children:[Object(g.jsx)(R,{src:l.a}),"Ebook"]})]})]})}},67:function(A,i,e){"use strict";var t=e(25);const a=e(66).a.div`
  background-color: ${t.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=a},68:function(A,i,e){"use strict";e.d(i,"b",(function(){return t})),e.d(i,"d",(function(){return a})),e.d(i,"e",(function(){return c})),e.d(i,"a",(function(){return o})),e.d(i,"h",(function(){return g})),e.d(i,"f",(function(){return E})),e.d(i,"g",(function(){return n})),e.d(i,"c",(function(){return C}));const t=A=>({type:"SET_CURRENT_STEP",payload:A}),a=A=>({type:"SET_SELECTED_MODULE",payload:A}),c=A=>({type:"SET_SELECTED_STEP",payload:A}),o=A=>({type:"ADD_STEP",payload:A}),g=A=>({type:"UPDATE_STEP",payload:A}),E=A=>({type:"SET_STEPS",payload:A}),n=A=>({type:"SET_TITLE",payload:A}),C=A=>({type:"SET_DESCRIPTION",payload:A})},69:function(A,i,e){"use strict";i.a=e.p+"static/media/white mtn.bb2904a1.png"},70:function(A,i,e){"use strict";e.d(i,"d",(function(){return o})),e.d(i,"a",(function(){return g})),e.d(i,"f",(function(){return E})),e.d(i,"g",(function(){return n})),e.d(i,"h",(function(){return C})),e.d(i,"e",(function(){return s})),e.d(i,"b",(function(){return I})),e.d(i,"c",(function(){return B}));var t=e(13),a=e(22);const c=Object(t.a)((()=>a.c.getState()),(A=>A.module)),o=Object(t.a)(c,(A=>A.inModule)),g=Object(t.a)(c,(A=>A.currentStep)),E=Object(t.a)(c,(A=>A.selectedModule)),n=Object(t.a)(c,(A=>A.selectedStep)),C=Object(t.a)(E,(A=>null===A||void 0===A?void 0:A.steps)),s=Object(t.a)(C,(A=>0===A.length?0:Math.max(...A.map((A=>A.id)))+1)),I=Object(t.a)(g,C,((A,i)=>i[A].data)),B=Object(t.a)(g,C,((A,i)=>(A+1)/(i.length+1)*100))},71:function(A,i,e){"use strict";e(2);var t=e(66),a=e(25),c=e(75),o=e(5);const g=t.a.img`
  height: 50%;
  border-radius: 50%;
`;var E=()=>Object(o.jsx)(g,{src:"data:image/png;base64,"}),n=e(76),C=e(16),s=e(11),I=e(21),B=e(77);const r=t.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,Q=Object(t.a)(B.a)`
  height: 50%;
`,d=t.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 800px) {
    display: none;
  }
`,h=t.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,l=t.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var p=({className:A})=>{const i=Object(C.c)(I.c),e=Object(C.c)(I.a),t=Object(s.g)();return Object(o.jsxs)(r,{className:A,onClick:()=>t.push("/profile"),children:[Object(o.jsx)(Q,{}),Object(o.jsxs)(d,{children:[Object(o.jsx)(h,{"data-test":"display-name",children:i}),Object(o.jsx)(l,{"data-test":"account-type",children:e})]})]})},x=(e(37),e(22));const b=t.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${a.a.HEADER_HEIGHT};
  left: ${a.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${a.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,u=t.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,R=t.a.div`
  width: 50%;
  height: 100%;
`,j=t.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,J=t.a.img`
  height: 45%;
`,O=t.a.img`
  height: 40%;
`,k=t.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,S=Object(t.a)(p)``;i.a=()=>{const A=Object(C.b)();return Object(o.jsxs)(b,{id:"header",children:[Object(o.jsx)(R,{}),Object(o.jsxs)(u,{children:[Object(o.jsx)(S,{}),Object(o.jsxs)(j,{children:[Object(o.jsx)(J,{src:c.a}),Object(o.jsx)(O,{src:"data:image/png;base64,"})]}),Object(o.jsx)(E,{}),Object(o.jsx)(k,{onClick:()=>A(Object(x.a)()),src:n.a})]})]})}},72:function(A,i,e){"use strict";e(2);var t=e(66),a=e(25),c=e(69),o=e(11),g=e(5);const E=t.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${a.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${a.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,n=t.a.div`
  background-color: ${a.a.COLOR.GREEN};
  height: ${a.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,C=t.a.img`
  width: 60px;
`,s=t.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(o.g)();return Object(g.jsxs)(E,{children:[Object(g.jsx)(n,{children:Object(g.jsx)(C,{onClick:()=>A.push("/home"),src:c.a})}),Object(g.jsx)(s,{src:"data:image/png;base64,"}),Object(g.jsx)(s,{src:"data:image/png;base64,"}),Object(g.jsx)(s,{src:"data:image/png;base64,"})]})}},75:function(A,i,e){"use strict";i.a="data:image/png;base64,"},76:function(A,i,e){"use strict";i.a="data:image/png;base64,"},77:function(A,i,e){"use strict";e(2);var t=e(66),a=e.p+"static/media/default profile icon.591db299.png",c=e(5);const o=t.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(c.jsx)(o,{className:A,src:a})}}]);
//# sourceMappingURL=3.9d61af04.chunk.js.map