(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[9],{126:function(A,e,t){"use strict";var i=t(2),c=t(16),a=t(11),o=t(66),n=t(68),s=t(69),r=t(25),d=t(93),g=t(94),E=t(95),C=t(96),l=t(5);const I=o.a.div`
  background-color: ${r.a.COLOR.GREEN};
  border-radius: 4px;
  width: 70%;
  height: 90px;
  margin: 8px 0px;
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 10px 30px;
`,B=o.a.p`
  text-align: center;
  font-size: 30px;
  color: white;
`,p=o.a.img`
  height: 90%;
`;var h=({id:A,type:e,handleDrag:t,handleDrop:i,onClick:c})=>{const a=()=>{switch(e){case"text":return Object(l.jsx)(p,{src:d.a});case"quiz":return Object(l.jsx)(p,{src:g.a});case"ebook":return Object(l.jsx)(p,{src:C.a});case"video":return Object(l.jsx)(p,{src:E.a});default:return Object(l.jsx)(l.Fragment,{})}};return Object(l.jsxs)(I,{draggable:!0,id:A,onDragOver:A=>A.preventDefault(),onDragStart:t,onDrop:i,onClick:c,children:[Object(l.jsx)(a,{}),Object(l.jsx)(B,{children:e.toUpperCase()})]})};const x=o.a.div`
  display: flex;
  flex-direction: column;
  margin: 40px 0px;
`;e.a=A=>{const e=Object(c.b)(),t=Object(a.g)(),[o,r]=Object(i.useState)(),d=Object(c.c)(s.h),g=A=>{r(A.currentTarget.id)},E=A=>{const t=d.find((A=>A.id===parseInt(o))),i=d.find((e=>e.id===parseInt(A.currentTarget.id))),c=t.order,a=i.order,s=d.map((e=>(e.id===parseInt(o)&&(e.order=a),e.id===parseInt(A.currentTarget.id)&&(e.order=c),e)));e(Object(n.f)(s))};return Object(l.jsx)(x,{"data-test":"step-container",children:d.sort(((A,e)=>A.order-e.order)).map((A=>Object(l.jsx)(h,{id:parseInt(A.id),boxColor:A.color,type:A.type,handleDrag:g,handleDrop:E,onClick:()=>{return i=A,e(Object(n.e)(i)),void t.push(`/teacher/create-module/edit-${i.type}`);var i},"data-test":"box"},parseInt(A.id))))})}},127:function(A,e,t){"use strict";t(2);var i=t(16),c=t(66),a=t(25),o=t(67),n=t(5);const s=c.a.div`
  background-color: white;
  border-radius: 4px;
  width: 40%;
  margin: 8px 0px;
  display: flex;
  align-items: center;
  position: relative;
  justify-content: center;
  padding: 30px;
`,r=c.a.p`
  top: 10px;
  right: 20px;
  position: absolute;
  :hover {
    cursor: pointer;
  }
`,d=c.a.div`
  height: 100vh;
  width: 100vw;
  display: flex;
  position: fixed;
  z-index: 10000;
  align-items: center;
  justify-content: center;
  top: 0;
  background: rgba(0, 0, 0, 0.6);
`,g=c.a.div`
  width: 100%;
`;var E=A=>Object(n.jsx)(d,{style:A.style,children:Object(n.jsxs)(s,{className:A.className,children:[Object(n.jsx)(r,{onClick:A.onClose,children:"X"}),Object(n.jsx)(g,{children:A.children})]})}),C=t(69),l=t(11),I=t(93),B=t(94),p=t(95),h=t(96),x=t(21),u=t(68);const Q=Object(c.a)(E)``,b=c.a.p`
  text-align: center;
  font-size: 30px;
`,j=c.a.img`
  height: 90%;
`,O=c.a.p`
  @media (max-width: 500px) {
    display: none;
  }
`,R=c.a.div`
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: space-between;
  width: 100%;
`,m=Object(c.a)(o.a)`
  background-color: ${a.a.COLOR.GREEN};
  border-radius: 4px;
  width: 50%;
  height: 90px;
  margin: 8px 0px;
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 10px 30px;
  padding-right: 10%;
  font-size: 50px;
`;e.a=({className:A,onClose:e,style:t})=>{const c=Object(l.g)(),a=Object(i.b)(),o=(Object(i.c)(C.h),Object(i.c)(x.a)),s=A=>{a(Object(u.e)(void 0)),c.push(`/${o}/create-module/edit-${A}`)};return Object(n.jsxs)(Q,{style:t,className:A,onClose:e,children:[Object(n.jsx)(b,{children:"Select Step to Add"}),Object(n.jsxs)(R,{children:[Object(n.jsxs)(m,{onClick:()=>s("text"),"data-test":"add-text",children:[Object(n.jsx)(j,{src:I.a}),Object(n.jsx)(O,{children:"Text"})]}),Object(n.jsxs)(m,{onClick:()=>s("video"),"data-test":"add-video",children:[Object(n.jsx)(j,{src:p.a}),Object(n.jsx)(O,{children:"Video"})]}),Object(n.jsxs)(m,{onClick:()=>s("quiz"),"data-test":"add-quiz",children:[Object(n.jsx)(j,{src:B.a}),Object(n.jsx)(O,{children:"Quiz"})]}),"admin"===o&&Object(n.jsxs)(m,{onClick:()=>s("ebook"),"data-test":"add-ebook",children:[Object(n.jsx)(j,{src:h.a}),"Ebook"]})]})]})}},238:function(A,e,t){"use strict";t.r(e);var i=t(2),c=t(36),a=t(73),o=t(72),n=t(66),s=t(25),r=t(19),d=t(23),g=t(11),E=t(67),C=t(126),l=t(127),I=t(16),B=t(69),p=t(68),h=t(37),x=t(5);const u=n.a.div`
  margin-top: ${s.a.HEADER_HEIGHT};
  margin-left: ${s.a.SIDEBAR_WIDTH};
  padding: 10px 50px;
`,Q=n.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,b=n.a.p`
  font-size: 20px;
`,j=n.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,O=n.a.div``,R=n.a.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
`,m=n.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,J=n.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,k=Object(n.a)(E.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,S=Object(n.a)(k)``,f=Object(n.a)(E.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,U=n.a.p`
  color: red;
  margin-top: -20px;
`,K=n.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;e.default=()=>{const A=Object(g.g)(),e=Object(I.b)(),[t,n]=Object(i.useState)(!1),[s,E]=Object(i.useState)(),Y=Object(I.c)(B.f),v=Object(I.c)(B.h);Object(i.useEffect)((()=>{}),[v]);return Object(x.jsxs)(c.a,{title:"Create a Module",description:"Create a Module",children:[Object(x.jsx)(a.a,{}),Object(x.jsx)(o.a,{}),Object(x.jsx)(l.a,{style:{display:t?"flex":"none"},onClose:()=>n(!1)}),Object(x.jsxs)(u,{children:[Object(x.jsx)(Q,{children:"Create New Module"}),Object(x.jsx)(b,{children:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce ac purus eget lacus pellentesque consequat. Donec commodo tincidunt lorem, c ursus accumsan eros varius vel."}),Object(x.jsxs)(m,{id:"moduleForm",children:[Object(x.jsx)(K,{children:"Module Title"}),Object(x.jsx)(J,{type:"text",id:"title",name:"title",defaultValue:null===Y||void 0===Y?void 0:Y.title,"data-test":"title",required:!0}),Object(x.jsx)(K,{children:"Module Description"}),Object(x.jsx)(J,{type:"text",id:"description",name:"description","data-test":"description",defaultValue:null===Y||void 0===Y?void 0:Y.description}),Object(x.jsxs)(R,{children:[Object(x.jsx)(K,{children:"Steps - Drag to Change Order"}),Object(x.jsx)(S,{onClick:()=>{e(Object(p.g)(document.getElementById("title").value)),e(Object(p.c)(document.getElementById("description").value)),n(!0)},"data-test":"add-step",children:"Add Step"})]}),Object(x.jsx)(O,{children:Object(x.jsx)(C.a,{steps:v})}),Object(x.jsx)(U,{children:s}),Object(x.jsxs)(j,{children:[Object(x.jsx)(k,{onClick:()=>{E();const t=Object.values(document.forms.moduleForm).filter((A=>A.name)).reduce(((A,e)=>(A[e.name]=e.value,A)),{}),i={...Y,title:t.title,description:t.description,creator:r.b.currentUser.uid};""!==t.title?0!==v.length?Y.creator?r.a.doc(Y.mid).update(i).then((()=>{r.e.doc(r.b.currentUser.uid).get().then((A=>{const e=A.data().moduleList.filter((A=>A.mid!==Y.mid));r.e.doc(r.b.currentUser.uid).update({moduleList:[...e,{mid:i.mid,title:t.title,description:t.description}]})})).then((()=>{r.e.doc(r.b.currentUser.uid).get().then((A=>e(Object(h.b)(A.data()))))}))})).then((()=>{r.e.doc(r.b.currentUser.uid).get().then((A=>e(Object(h.b)(A.data())))),A.push("admin/home")})):r.a.add(i).then((A=>{r.a.doc(A.id).update({mid:A.id}),r.e.doc(r.b.currentUser.uid).update({moduleList:d.a.firestore.FieldValue.arrayUnion({mid:A.id,title:t.title,description:t.description})})})).then((()=>{r.e.doc(r.b.currentUser.uid).get().then((A=>e(Object(h.b)(A.data())))),A.push("admin/home")})):E("Please add at least one step"):E("Please fill out module title")},"data-test":"create",children:"CREATE"}),Object(x.jsx)(f,{onClick:()=>A.push("admin/home"),children:"BACK TO DASHBOARD"})]})]})]})]})}},67:function(A,e,t){"use strict";var i=t(25);const c=t(66).a.div`
  background-color: ${i.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;e.a=c},68:function(A,e,t){"use strict";t.d(e,"b",(function(){return i})),t.d(e,"d",(function(){return c})),t.d(e,"e",(function(){return a})),t.d(e,"a",(function(){return o})),t.d(e,"h",(function(){return n})),t.d(e,"f",(function(){return s})),t.d(e,"g",(function(){return r})),t.d(e,"c",(function(){return d}));const i=A=>({type:"SET_CURRENT_STEP",payload:A}),c=A=>({type:"SET_SELECTED_MODULE",payload:A}),a=A=>({type:"SET_SELECTED_STEP",payload:A}),o=A=>({type:"ADD_STEP",payload:A}),n=A=>({type:"UPDATE_STEP",payload:A}),s=A=>({type:"SET_STEPS",payload:A}),r=A=>({type:"SET_TITLE",payload:A}),d=A=>({type:"SET_DESCRIPTION",payload:A})},69:function(A,e,t){"use strict";t.d(e,"d",(function(){return o})),t.d(e,"a",(function(){return n})),t.d(e,"f",(function(){return s})),t.d(e,"g",(function(){return r})),t.d(e,"h",(function(){return d})),t.d(e,"e",(function(){return g})),t.d(e,"b",(function(){return E})),t.d(e,"c",(function(){return C}));var i=t(13),c=t(22);const a=Object(i.a)((()=>c.c.getState()),(A=>A.module)),o=Object(i.a)(a,(A=>A.inModule)),n=Object(i.a)(a,(A=>A.currentStep)),s=Object(i.a)(a,(A=>A.selectedModule)),r=Object(i.a)(a,(A=>A.selectedStep)),d=Object(i.a)(s,(A=>null===A||void 0===A?void 0:A.steps)),g=Object(i.a)(d,(A=>0===A.length?0:Math.max(...A.map((A=>A.id)))+1)),E=Object(i.a)(n,d,((A,e)=>e[A].data)),C=Object(i.a)(n,d,((A,e)=>(A+1)/(e.length+1)*100))},72:function(A,e,t){"use strict";t(2);var i=t(66),c=t(25),a=t(76),o=t(5);const n=i.a.img`
  height: 50%;
  border-radius: 50%;
`;var s=()=>Object(o.jsx)(n,{src:"data:image/png;base64,"}),r=t(77),d=t(16),g=t(11),E=t(21),C=t(78);const l=i.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,I=Object(i.a)(C.a)`
  height: 50%;
`,B=i.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 400px) {
    display: none;
  }
`,p=i.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,h=i.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var x=({className:A})=>{const e=Object(d.c)(E.c),t=Object(d.c)(E.a),i=Object(g.g)();return Object(o.jsxs)(l,{className:A,onClick:()=>i.push("/profile"),children:[Object(o.jsx)(I,{}),Object(o.jsxs)(B,{children:[Object(o.jsx)(p,{"data-test":"display-name",children:e}),Object(o.jsx)(h,{"data-test":"account-type",children:t})]})]})},u=(t(37),t(22));const Q=i.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${c.a.HEADER_HEIGHT};
  left: ${c.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${c.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,b=i.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,j=i.a.div`
  width: 50%;
  height: 100%;
`,O=i.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,R=i.a.img`
  height: 45%;
`,m=i.a.img`
  height: 40%;
`,J=i.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,k=Object(i.a)(x)``;e.a=()=>{const A=Object(d.b)();return Object(o.jsxs)(Q,{id:"header",children:[Object(o.jsx)(j,{}),Object(o.jsxs)(b,{children:[Object(o.jsx)(k,{}),Object(o.jsxs)(O,{children:[Object(o.jsx)(R,{src:a.a}),Object(o.jsx)(m,{src:"data:image/png;base64,"})]}),Object(o.jsx)(s,{}),Object(o.jsx)(J,{onClick:()=>A(Object(u.a)()),src:r.a})]})]})}},73:function(A,e,t){"use strict";t(2);var i=t(66),c=t(25),a=t(74),o=t(11),n=t(5);const s=i.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${c.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${c.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,r=i.a.div`
  background-color: ${c.a.COLOR.GREEN};
  height: ${c.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,d=i.a.img`
  width: 60px;
`,g=i.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;e.a=()=>{const A=Object(o.g)();return Object(n.jsxs)(s,{children:[Object(n.jsx)(r,{children:Object(n.jsx)(d,{onClick:()=>A.push("/home"),src:a.a})}),Object(n.jsx)(g,{src:"data:image/png;base64,"}),Object(n.jsx)(g,{src:"data:image/png;base64,"}),Object(n.jsx)(g,{src:"data:image/png;base64,"})]})}},74:function(A,e,t){"use strict";e.a=t.p+"static/media/white mtn.bb2904a1.png"},76:function(A,e,t){"use strict";e.a="data:image/png;base64,"},77:function(A,e,t){"use strict";e.a="data:image/png;base64,"},78:function(A,e,t){"use strict";t(2);var i=t(66),c=t.p+"static/media/default profile icon.591db299.png",a=t(5);const o=i.a.img`
  height: 50%;
  border-radius: 50%;
`;e.a=({className:A})=>Object(a.jsx)(o,{className:A,src:c})},93:function(A,e,t){"use strict";e.a=t.p+"static/media/text icon.b8202f2f.png"},94:function(A,e,t){"use strict";e.a=t.p+"static/media/quiz icon.08cab1b6.png"},95:function(A,e,t){"use strict";e.a="data:image/png;base64,"},96:function(A,e,t){"use strict";e.a=t.p+"static/media/ebook icon.7e0b328b.png"}}]);
//# sourceMappingURL=9.241b8b69.chunk.js.map