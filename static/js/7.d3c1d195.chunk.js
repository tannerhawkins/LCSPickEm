(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[7],{103:function(A,i,e){"use strict";e.r(i);var t=e(2),a=e(36),c=e(82),o=e(81),s=e(76),g=e(25),n=e(77),E=e(11),C=e(20),B=e(88),d=e(18),l=e(87),r=e(21),I=e(23),h=e(5);const Q=s.a.div`
  margin-top: ${g.a.HEADER_HEIGHT};
  margin-left: ${g.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,x=s.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,u=s.a.p`
  font-size: 20px;
`,R=s.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,m=s.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,j=s.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,p=Object(s.a)(n.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${g.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,b=Object(s.a)(n.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${g.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,O=s.a.p`
  color: red;
`,J=s.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;i.default=()=>{const A=Object(E.g)(),[i,e]=Object(t.useState)(),s=Object(d.c)(l.a);Object(d.c)(r.b);return Object(h.jsxs)(a.a,{title:"Add a Student",description:"Add a Student",children:[Object(h.jsx)(c.a,{}),Object(h.jsx)(o.a,{}),Object(h.jsxs)(Q,{children:[Object(h.jsx)(x,{children:"Add Students to a Class"}),Object(h.jsx)(u,{children:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce ac purus eget lacus pellentesque consequat. Donec commodo tincidunt lorem, c ursus accumsan eros varius vel."}),Object(h.jsxs)(m,{id:"studentsForm",children:[Object(h.jsx)(J,{children:"Student's Email"}),Object(h.jsx)(j,{type:"text",name:"email","data-test":"email",required:!0}),Object(h.jsx)(J,{children:"Add to Class"}),Object(h.jsx)(B.a,{}),Object(h.jsx)(O,{"data-test":"error",children:i}),Object(h.jsxs)(R,{children:[Object(h.jsx)(p,{onClick:()=>{e();const A=Object.values(document.forms.studentsForm).reduce(((A,i)=>(A[i.name]=i.value,A)),{});""!==A.email&&A.email.includes("@")?C.d.where("email","==",A.email).get().then((A=>{var i;const t=null===(i=A.docs[0])||void 0===i?void 0:i.data();void 0!==t&&"student"===t.accountType?t.classList.map((A=>A.cid)).includes(s.cid)?e("Student already in class"):(C.b.doc(s.cid).update({students:I.a.firestore.FieldValue.arrayUnion({uid:t.uid,displayName:t.displayName,email:t.email})}),C.d.doc(t.uid).update({classList:I.a.firestore.FieldValue.arrayUnion({cid:s.cid,modules:s.modules,className:s.className})}),e(`Added ${t.displayName} to ${s.className}`),document.getElementById("studentsForm").reset()):e("Student not found")})):e("Please enter a valid email")},"data-test":"submit",children:"ADD TO CLASS"}),Object(h.jsx)(b,{onClick:()=>A.push("teacher/home"),"data-test":"back",children:"BACK TO DASHBOARD"})]})]})]})]})}},77:function(A,i,e){"use strict";var t=e(25);const a=e(76).a.div`
  background-color: ${t.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=a},78:function(A,i,e){"use strict";i.a=e.p+"static/media/white mtn.bb2904a1.png"},79:function(A,i,e){"use strict";i.a="data:image/png;base64,"},80:function(A,i,e){"use strict";i.a="data:image/png;base64,"},81:function(A,i,e){"use strict";e(2);var t=e(76),a=e(25),c="data:image/png;base64,",o=e(79),s=e(5);const g=t.a.img`
  height: 50%;
  border-radius: 50%;
`;var n=()=>Object(s.jsx)(g,{src:"data:image/png;base64,"}),E=e(80),C=e(18),B=e(21),d=e(83);const l=t.a.div`
  height: 100%;
  display: flex;
  align-items: center;
`,r=t.a.img`
  height: 40%;
`,I=Object(t.a)(d.a)`
  height: 50%;
`,h=t.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 10px;

  @media (max-width: 800px) {
    display: none;
  }
`,Q=t.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,x=t.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var u=({className:A})=>{const i=Object(C.c)(B.c),e=Object(C.c)(B.a);return Object(s.jsxs)(l,{className:A,children:[Object(s.jsx)(I,{}),Object(s.jsx)(r,{src:c}),Object(s.jsxs)(h,{children:[Object(s.jsx)(Q,{"data-test":"display-name",children:i}),Object(s.jsx)(x,{"data-test":"account-type",children:e})]})]})},R=(e(37),e(22));const m=t.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${a.a.HEADER_HEIGHT};
  left: ${a.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${a.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,j=t.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,p=t.a.div`
  width: 50%;
  height: 100%;
`,b=t.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,O=t.a.img`
  height: 45%;
`,J=t.a.img`
  height: 40%;
`,Y=t.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,U=Object(t.a)(u)``;i.a=()=>{const A=Object(C.b)();return Object(s.jsxs)(m,{id:"header",children:[Object(s.jsx)(p,{}),Object(s.jsxs)(j,{children:[Object(s.jsx)(U,{}),Object(s.jsxs)(b,{children:[Object(s.jsx)(O,{src:o.a}),Object(s.jsx)(J,{src:c})]}),Object(s.jsx)(n,{}),Object(s.jsx)(Y,{onClick:()=>A(Object(R.a)()),src:E.a})]})]})}},82:function(A,i,e){"use strict";e(2);var t=e(76),a=e(25),c=e(78),o=e(11),s=e(5);const g=t.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${a.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${a.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,n=t.a.div`
  background-color: ${a.a.COLOR.GREEN};
  height: ${a.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,E=t.a.img`
  width: 60px;
`,C=t.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(o.g)();return Object(s.jsxs)(g,{children:[Object(s.jsx)(n,{children:Object(s.jsx)(E,{onClick:()=>A.push("/home"),src:c.a})}),Object(s.jsx)(C,{src:"data:image/png;base64,"}),Object(s.jsx)(C,{src:"data:image/png;base64,"}),Object(s.jsx)(C,{src:"data:image/png;base64,"})]})}},83:function(A,i,e){"use strict";e(2);var t=e(76),a=e.p+"static/media/default profile icon.591db299.png",c=e(5);const o=t.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(c.jsx)(o,{className:A,src:a})},86:function(A,i,e){"use strict";e.d(i,"a",(function(){return t}));e(20);const t=A=>({type:"SET_CURRENT_CLASS",payload:A})},87:function(A,i,e){"use strict";e.d(i,"a",(function(){return o}));var t=e(17),a=e(22);const c=Object(t.a)((()=>a.c.getState()),(A=>A.class)),o=Object(t.a)(c,(A=>A.currentClass))},88:function(A,i,e){"use strict";var t=e(2),a=e(18),c=e(76),o=e(21),s=e(86),g=e(87),n=e(25),E=e(20),C=e(5);const B=c.a.form`
  display: flex;
  flex-flow: column;
  justify-content: flex-start;
`,d=c.a.select`
  height: 100%;
  padding: 0.5rem;
  border: 0px;
  outline: 0px;
  font-size: 25px;
  color: ${n.a.COLOR.GREEN};
  background-color: ${n.a.COLOR.GRAY};
`,l=c.a.option`
  color: ${A=>A.defaultValue?"lightgrey":"black"};
`;i.a=A=>{const i=Object(a.b)(),e=Object(a.c)(o.b),c=Object(a.c)(g.a);Object(t.useEffect)((()=>{}));return e[0]?Object(C.jsx)(B,{action:A.action,"data-test":"dropdown",children:Object(C.jsx)(d,{id:"services","data-test":"class",name:"services",onChange:A=>{const e=Array.from(A.target.children).filter((A=>A.selected)).map((A=>A.dataset.cid))[0];E.b.doc(e).get().then((A=>i(Object(s.a)(A.data()))))},defaultValue:null===c||void 0===c?void 0:c.cid,children:e.map((A=>Object(C.jsx)(l,{value:A.cid,"data-cid":A.cid,classItem:A,children:A.className},A.cid)))})}):Object(C.jsx)(C.Fragment,{})}}}]);
//# sourceMappingURL=7.d3c1d195.chunk.js.map