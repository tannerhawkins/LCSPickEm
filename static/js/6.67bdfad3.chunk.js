(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[6],{48:function(A,i,e){"use strict";e.d(i,"a",(function(){return t}));const t={COLOR:{LIGHT_GREEN:"#A2E1D0",DARK_GREEN:"#6BAE9B",GREEN:"#7DCBB5",WHITE:"#e9e9ff",DARK_BLUE:"#373B53",TEAL:"#B5FFEA"},HEADER_HEIGHT:"80px",SIDEBAR_WIDTH:"80px"}},52:function(A,i,e){"use strict";var t=e(48);const a=e(47).a.div`
  background-color: ${t.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=a},54:function(A,i,e){"use strict";e.d(i,"a",(function(){return t})),e.d(i,"b",(function(){return a}));const t=A=>({type:"SIGN_IN",payload:A}),a=()=>({type:"SIGN_OUT"})},55:function(A,i,e){"use strict";i.a=e.p+"static/media/white mtn.bb2904a1.png"},56:function(A,i,e){"use strict";e.d(i,"c",(function(){return c})),e.d(i,"b",(function(){return s})),e.d(i,"a",(function(){return o}));var t=e(59);e(60);t.a.initializeApp({apiKey:"AIzaSyBikWf9vy86sF9wXyRjkpdrrAJaw7WoQ8M",authDomain:"ancient-path-adventures.firebaseapp.com",projectId:"ancient-path-adventures",storageBucket:"ancient-path-adventures.appspot.com",messagingSenderId:"1051282870332",appId:"1:1051282870332:web:8f7a5922db7d370e86b339",measurementId:"G-NPPQJQTEQN"});const a=t.a.firestore();var o=t.a.auth();new t.a.auth.GoogleAuthProvider;const c=a.collection("userData"),s=a.collection("classData")},58:function(A,i,e){"use strict";e.d(i,"a",(function(){return t}));const t=A=>({type:"SET_CURRENT_CLASS",payload:A})},62:function(A,i,e){"use strict";i.a="data:image/png;base64,"},64:function(A,i,e){"use strict";i.a="data:image/png;base64,"},65:function(A,i,e){"use strict";e(0);var t=e(47),a=e(48),o="data:image/png;base64,",c=e(62),s=e(2);const n=t.a.img`
  height: 50%;
  border-radius: 50%;
`;var E=()=>Object(s.jsx)(n,{src:"data:image/png;base64,"}),g=e(64),C=e(11),B=e(14),r=e(67);const I=t.a.div`
  height: 100%;
  display: flex;
  align-items: center;
`,h=t.a.img`
  height: 40%;
`,Q=Object(t.a)(r.a)`
  height: 50%;
`,l=t.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 10px;

  @media (max-width: 800px) {
    display: none;
  }
`,d=t.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,x=t.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var R=({className:A})=>{const i=Object(C.c)(B.c),e=Object(C.c)(B.a);return Object(s.jsxs)(I,{className:A,children:[Object(s.jsx)(Q,{}),Object(s.jsx)(h,{src:o}),Object(s.jsxs)(l,{children:[Object(s.jsx)(d,{children:i}),Object(s.jsx)(x,{children:e})]})]})},p=e(54);const u=t.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${a.a.HEADER_HEIGHT};
  left: ${a.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${a.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,b=t.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,j=t.a.div`
  width: 50%;
  height: 100%;
`,J=t.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,m=t.a.img`
  height: 45%;
`,Y=t.a.img`
  height: 40%;
`,U=t.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,O=Object(t.a)(R)``;i.a=()=>{const A=Object(C.b)();return Object(s.jsxs)(u,{id:"header",children:[Object(s.jsx)(j,{}),Object(s.jsxs)(b,{children:[Object(s.jsx)(O,{}),Object(s.jsxs)(J,{children:[Object(s.jsx)(m,{src:c.a}),Object(s.jsx)(Y,{src:o})]}),Object(s.jsx)(E,{}),Object(s.jsx)(U,{onClick:()=>A(Object(p.b)()),src:g.a})]})]})}},66:function(A,i,e){"use strict";e(0);var t=e(47),a=e(48),o=e(55),c=e(5),s=e(2);const n=t.a.div`
  display: flex;
  flex-direction: column;
  height: calc(100% + ${a.a.HEADER_HEIGHT});
  width: ${a.a.SIDEBAR_WIDTH};
  position: absolute;
  left: 0;
  top: 0;
  background-color: ${a.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,E=t.a.div`
  background-color: ${a.a.COLOR.GREEN};
  height: ${a.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,g=t.a.img`
  width: 60px;
`,C=t.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(c.g)();return Object(s.jsxs)(n,{children:[Object(s.jsx)(E,{children:Object(s.jsx)(g,{onClick:()=>A.push("/home"),src:o.a})}),Object(s.jsx)(C,{src:"data:image/png;base64,"}),Object(s.jsx)(C,{src:"data:image/png;base64,"}),Object(s.jsx)(C,{src:"data:image/png;base64,"})]})}},67:function(A,i,e){"use strict";e(0);var t=e(47),a=e.p+"static/media/default profile icon.591db299.png",o=e(2);const c=t.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(o.jsx)(c,{className:A,src:a})},84:function(A,i,e){"use strict";e.r(i);var t=e(0),a=e(23),o=e(66),c=e(65),s=e(47),n=e(48),E=e(52),g=e(5),C=e(11),B=e(14),r=e(56),I=e(59),h=e(58),Q=e(54),l=e(2);const d=s.a.div`
  margin-top: ${n.a.HEADER_HEIGHT};
  margin-left: ${n.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,x=s.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,R=s.a.p`
  font-size: 20px;
`,p=s.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,u=s.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,b=s.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,j=Object(s.a)(E.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${n.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,J=Object(s.a)(E.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${n.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,m=s.a.p`
  color: red;
  margin-top: -20px;
`,Y=s.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;i.default=()=>{const A=Object(g.g)(),i=Object(C.c)(B.a),e=(Object(C.c)(B.b),Object(C.b)()),[s,n]=Object(t.useState)();return Object(l.jsxs)(a.a,{title:"Create a Class",description:"Create a Class",children:[Object(l.jsx)(o.a,{}),Object(l.jsx)(c.a,{}),Object(l.jsxs)(d,{children:[Object(l.jsx)(x,{children:"Create new class"}),Object(l.jsx)(R,{children:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce ac purus eget lacus pellentesque consequat. Donec commodo tincidunt lorem, c ursus accumsan eros varius vel."}),Object(l.jsxs)(u,{id:"classForm",children:[Object(l.jsx)(Y,{children:"Class Name"}),Object(l.jsx)(b,{type:"text",name:"name",required:!0}),Object(l.jsx)(Y,{children:"Class Description"}),Object(l.jsx)(b,{type:"text",name:"description"}),Object(l.jsx)(m,{children:s}),Object(l.jsxs)(p,{children:[Object(l.jsx)(j,{onClick:()=>{n();const t=Object.values(document.forms.classForm).reduce(((A,i)=>(A[i.name]=i.value,A)),{});""!==t.name?r.b.add({className:t.name,description:t.description,students:[],enrollmentCode:"placeholdercode"}).then((a=>{r.b.doc(a.id).update({cid:a.id}).then((()=>{r.b.doc(a.id).get().then((A=>{e(Object(h.a)(A.data()))}))})),r.c.doc(r.a.currentUser.uid).update({classList:I.a.firestore.FieldValue.arrayUnion({cid:a.id,className:t.name})}).then((()=>{r.c.doc(r.a.currentUser.uid).get().then((A=>{e(Object(Q.a)(A.data()))})),A.push(`${i}/home`)}))})):n("Please fill out class name")},children:"CREATE"}),Object(l.jsx)(J,{onClick:()=>A.push(`${i}/home`),children:"BACK TO DASHBOARD"})]})]})]})]})}}}]);
//# sourceMappingURL=6.67bdfad3.chunk.js.map