(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[15],{233:function(A,i,e){"use strict";e.r(i);var t=e(2),o=e(36),a=e(72),c=e(73),g=e(66),s=e(25),C=e(67),E=e(11),B=e(19),n=e(16),I=e(21),h=e(23),d=e(37),l=e(30),Q=e(5);const r=g.a.div`
  margin-top: ${s.a.HEADER_HEIGHT};
  margin-left: ${s.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,x=g.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,R=g.a.p`
  font-size: 20px;
`,p=g.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,J=g.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,j=g.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,u=Object(g.a)(C.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,Y=Object(g.a)(C.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,m=g.a.p`
  color: red;
`,b=g.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;i.default=()=>{const A=Object(E.g)(),[i,e]=Object(t.useState)(),g=Object(n.b)(),s=Object(n.c)(I.j);return Object(Q.jsxs)(o.a,{title:"Add a Class",description:"Add a Class",children:[Object(Q.jsx)(a.a,{}),Object(Q.jsx)(c.a,{}),Object(Q.jsxs)(r,{children:[Object(Q.jsx)(x,{children:"Add a Class"}),Object(Q.jsx)(R,{children:"Please enter the enrollment code of the class you would like to enroll in."}),Object(Q.jsxs)(J,{id:"enrollmentForm",children:[Object(Q.jsx)(b,{children:"Enrollment Code"}),Object(Q.jsx)(j,{type:"text",name:"code","data-test":"enrollment-code",required:!0}),Object(Q.jsx)(m,{children:i}),Object(Q.jsxs)(p,{children:[Object(Q.jsx)(u,{onClick:()=>{e();const i=Object.values(document.forms.enrollmentForm).reduce(((A,i)=>(A[i.name]=i.value,A)),{});""!==i.code?B.c.where("cid","==",i.code).get().then((i=>{var t;const o=null===(t=i.docs[0])||void 0===t?void 0:t.data();void 0!==o?o.students.map((A=>A.uid)).includes(s.uid)?e("Student already in class"):(B.c.doc(o.cid).update({students:h.a.firestore.FieldValue.arrayUnion({uid:s.uid,displayName:s.displayName,email:s.email})}),B.e.doc(s.uid).update({classList:h.a.firestore.FieldValue.arrayUnion({cid:o.cid,modules:o.modules,apaModules:o.apaModules,className:o.className})}).then((()=>{e(`Added ${s.displayName} to ${o.className}`),B.e.doc(s.uid).get().then((A=>g(Object(d.b)(A.data())))),B.c.doc(o.cid).get().then((i=>{g(Object(l.a)(i.data())),A.push("/student/home")}))}))):e("Class not found")})):e("Please enter a valid code")},"data-test":"submit",children:"ADD CLASS"}),Object(Q.jsx)(Y,{onClick:()=>A.push("student/home"),children:"BACK TO DASHBOARD"})]})]})]})]})}},67:function(A,i,e){"use strict";var t=e(25);const o=e(66).a.div`
  background-color: ${t.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=o},72:function(A,i,e){"use strict";e(2);var t=e(66),o=e(25),a=e(74),c=e(11),g=e(5);const s=t.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${o.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${o.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,C=t.a.div`
  background-color: ${o.a.COLOR.GREEN};
  height: ${o.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,E=t.a.img`
  width: 60px;
`,B=t.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(c.g)();return Object(g.jsxs)(s,{children:[Object(g.jsx)(C,{children:Object(g.jsx)(E,{onClick:()=>A.push("/home"),src:a.a})}),Object(g.jsx)(B,{src:"data:image/png;base64,"}),Object(g.jsx)(B,{src:"data:image/png;base64,"}),Object(g.jsx)(B,{src:"data:image/png;base64,"})]})}},73:function(A,i,e){"use strict";e(2);var t=e(66),o=e(25),a=e(75),c=e(77),g=e(5);const s=t.a.img`
  height: 50%;
  border-radius: 50%;
`;var C=()=>Object(g.jsx)(s,{src:"data:image/png;base64,"}),E=e(78),B=e(79),n=e(16),I=(e(37),e(22));const h=t.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${o.a.HEADER_HEIGHT};
  left: ${o.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${o.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,d=t.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,l=t.a.div`
  width: 50%;
  height: 100%;
`,Q=t.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,r=t.a.img`
  height: 45%;
`,x=t.a.img`
  height: 40%;
`,R=t.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,p=Object(t.a)(B.a)``;i.a=()=>{const A=Object(n.b)();return Object(g.jsxs)(h,{id:"header",children:[Object(g.jsx)(l,{}),Object(g.jsxs)(d,{children:[Object(g.jsx)(p,{}),Object(g.jsxs)(Q,{children:[Object(g.jsx)(r,{src:c.a}),Object(g.jsx)(x,{src:a.a})]}),Object(g.jsx)(C,{}),Object(g.jsx)(R,{onClick:()=>A(Object(I.a)()),src:E.a})]})]})}},74:function(A,i,e){"use strict";i.a=e.p+"static/media/white mtn.bb2904a1.png"},75:function(A,i,e){"use strict";i.a="data:image/png;base64,"},77:function(A,i,e){"use strict";i.a="data:image/png;base64,"},78:function(A,i,e){"use strict";i.a="data:image/png;base64,"},79:function(A,i,e){"use strict";e(2);var t=e(16),o=e(11),a=e(66),c=e(21),g=(e(75),e(80)),s=e(5);const C=a.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,E=Object(a.a)(g.a)`
  height: 50%;
`,B=a.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 400px) {
    display: none;
  }
`,n=a.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,I=a.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;i.a=({className:A})=>{const i=Object(t.c)(c.c),e=Object(t.c)(c.a),a=Object(o.g)();return Object(s.jsxs)(C,{className:A,onClick:()=>a.push("/profile"),children:[Object(s.jsx)(E,{}),Object(s.jsxs)(B,{children:[Object(s.jsx)(n,{"data-test":"display-name",children:i}),Object(s.jsx)(I,{"data-test":"account-type",children:e})]})]})}},80:function(A,i,e){"use strict";e(2);var t=e(66),o=e.p+"static/media/default profile icon.591db299.png",a=e(5);const c=t.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(a.jsx)(c,{className:A,src:o})}}]);
//# sourceMappingURL=15.3c68999d.chunk.js.map