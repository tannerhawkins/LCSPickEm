(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[6],{125:function(A,i,t){"use strict";t.r(i);t(2);var e=t(36),o=t(70),a=t(69),c=t(64),g=t(25),E=t(20),n=t(82),s=t(18),C=t(22),r=t(76),d=t(75),B=t(23),h=t(65),I=t(11),l=t(74),Q=t(83),x=t(5);const p=c.a.div`
  margin-top: ${g.a.HEADER_HEIGHT};
  margin-left: ${g.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,u=c.a.p`
  font-size: 36px;
  line-height: 43px;
  font-weight: 500;
`,R=(c.a.div`
  display: flex;
  width: 100%;
  flex-wrap: no-wrap;
  overflow: auto;
  height: 300px;
`,c.a.div`
  display: flex;
  align-items: space-around;
  justify-content: space-around;
  width: 60%;
`),b=Object(c.a)(h.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  width: 220px;
  background-color: ${g.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`;i.default=()=>{const A=Object(s.b)(),i=Object(I.g)(),t=Object(s.c)(C.g),c=Object(s.c)(r.a),h=null===t||void 0===t?void 0:t.filter((A=>!(null===c||void 0===c?void 0:c.modules.includes(A.mid)))).map((A=>Object(x.jsx)(n.a,{onClick:()=>j(A.mid),module:A.mid},A.mid))),j=i=>{E.b.doc(c.cid).update({modules:B.a.firestore.FieldValue.arrayUnion(i)}).then(E.b.doc(c.cid).get().then((A=>{A.data().students.forEach((A=>{E.d.doc(A.uid).get().then((t=>{const e=t.data().classList,o=e.filter((A=>A.cid===c.cid))[0],a={...o,modules:[...o.modules,i]},g=(()=>{for(let A=0;A<e.length;A++)if(e[A].cid===o.cid)return A;return-1})();e.splice(g,1,a),E.d.doc(A.uid).update({classList:e})}))}))})).then(E.b.doc(null===c||void 0===c?void 0:c.cid).get().then((i=>A(Object(d.a)(i.data()))))))};return Object(x.jsxs)(e.a,{title:"Add a Module",description:"Add a Module",children:[Object(x.jsx)(o.a,{}),Object(x.jsx)(a.a,{}),Object(x.jsxs)(p,{children:[Object(x.jsx)(u,{children:"Existing Modules"}),Object(x.jsx)(Q.a,{moduleCards:h}),Object(x.jsxs)(R,{children:[Object(x.jsx)(b,{onClick:()=>{A(Object(l.d)(g.a.EMPTY_MODULE)),i.push("create-module")},"data-test":"create-module",children:"CREATE NEW MODULE"}),Object(x.jsx)(b,{onClick:()=>i.push("home"),"data-test":"back",children:"BACK TO DASHBOARD"})]})]})]})}},65:function(A,i,t){"use strict";var e=t(25);const o=t(64).a.div`
  background-color: ${e.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=o},66:function(A,i,t){"use strict";i.a=t.p+"static/media/white mtn.bb2904a1.png"},68:function(A,i,t){"use strict";i.a="data:image/png;base64,"},69:function(A,i,t){"use strict";t(2);var e=t(64),o=t(25),a=t(68),c=t(5);const g=e.a.img`
  height: 50%;
  border-radius: 50%;
`;var E=()=>Object(c.jsx)(g,{src:"data:image/png;base64,"}),n=t(71),s=t(18),C=t(11),r=t(22),d=t(72);const B=e.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,h=Object(e.a)(d.a)`
  height: 50%;
`,I=e.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 800px) {
    display: none;
  }
`,l=e.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,Q=e.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var x=({className:A})=>{const i=Object(s.c)(r.c),t=Object(s.c)(r.a),e=Object(C.g)();return Object(c.jsxs)(B,{className:A,onClick:()=>e.push("/profile"),children:[Object(c.jsx)(h,{}),Object(c.jsxs)(I,{children:[Object(c.jsx)(l,{"data-test":"display-name",children:i}),Object(c.jsx)(Q,{"data-test":"account-type",children:t})]})]})},p=(t(35),t(21));const u=e.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${o.a.HEADER_HEIGHT};
  left: ${o.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${o.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,R=e.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,b=e.a.div`
  width: 50%;
  height: 100%;
`,j=e.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,J=e.a.img`
  height: 45%;
`,O=e.a.img`
  height: 40%;
`,m=e.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,Y=Object(e.a)(x)``;i.a=()=>{const A=Object(s.b)();return Object(c.jsxs)(u,{id:"header",children:[Object(c.jsx)(b,{}),Object(c.jsxs)(R,{children:[Object(c.jsx)(Y,{}),Object(c.jsxs)(j,{children:[Object(c.jsx)(J,{src:a.a}),Object(c.jsx)(O,{src:"data:image/png;base64,"})]}),Object(c.jsx)(E,{}),Object(c.jsx)(m,{onClick:()=>A(Object(p.a)()),src:n.a})]})]})}},70:function(A,i,t){"use strict";t(2);var e=t(64),o=t(25),a=t(66),c=t(11),g=t(5);const E=e.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${o.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${o.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,n=e.a.div`
  background-color: ${o.a.COLOR.GREEN};
  height: ${o.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,s=e.a.img`
  width: 60px;
`,C=e.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(c.g)();return Object(g.jsxs)(E,{children:[Object(g.jsx)(n,{children:Object(g.jsx)(s,{onClick:()=>A.push("/home"),src:a.a})}),Object(g.jsx)(C,{src:"data:image/png;base64,"}),Object(g.jsx)(C,{src:"data:image/png;base64,"}),Object(g.jsx)(C,{src:"data:image/png;base64,"})]})}},71:function(A,i,t){"use strict";i.a="data:image/png;base64,"},72:function(A,i,t){"use strict";t(2);var e=t(64),o=t.p+"static/media/default profile icon.591db299.png",a=t(5);const c=e.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(a.jsx)(c,{className:A,src:o})},74:function(A,i,t){"use strict";t.d(i,"b",(function(){return e})),t.d(i,"d",(function(){return o})),t.d(i,"a",(function(){return a})),t.d(i,"e",(function(){return c})),t.d(i,"f",(function(){return g})),t.d(i,"c",(function(){return E}));const e=A=>({type:"SET_CURRENT_STEP",payload:A}),o=A=>({type:"SET_SELECTED_MODULE",payload:A}),a=A=>({type:"ADD_STEP",payload:A}),c=A=>({type:"SET_STEPS",payload:A}),g=A=>({type:"SET_TITLE",payload:A}),E=A=>({type:"SET_DESCRIPTION",payload:A})},75:function(A,i,t){"use strict";t.d(i,"a",(function(){return e}));t(20);const e=A=>({type:"SET_CURRENT_CLASS",payload:A})},76:function(A,i,t){"use strict";t.d(i,"a",(function(){return c}));var e=t(17),o=t(21);const a=Object(e.a)((()=>o.c.getState()),(A=>A.class)),c=Object(e.a)(a,(A=>A.currentClass))},82:function(A,i,t){"use strict";var e=t(2),o=t(64),a=t(25),c=t(20),g=t(5);const E=o.a.div`
  background-color: ${a.a.COLOR.DARK_GREEN};
  filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25));
  display: flex;
  flex-direction: column;
  align-items: center;
  text-transform: "capitalize";
  justify-content: center;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
    background: ${a.a.COLOR.LIGHT_GREEN};
    transform: scale(1.02);
  }
  width: 200px;
  height: 300px;
  padding: 10px;
  min-width: 200px;
  margin: 0px 10px;
  border-radius: 0.5em;
  transition: transform 0.45s ease, background 0.45s ease;
`,n=o.a.p`
  color: ${a.a.COLOR.LIGHT_GREEN}
  font-size: 80px !important;
  text-transform: 'capitalize' !important;
  color: #2E3C40;
  font-weight: 600;
  line-height: 1;
  padding-bottom: 10px;
  padding-top: 10px;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid 
  transition: color .45s ease, border .45s ease;
  font-weight: 600;
  line-height: 1;
  padding-bottom: .5em;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid #2E3C40;
`,s=o.a.p`
  font-size: 15px;
  background-color: white;
  padding: 20px 5px;
  width: 85%;
  height: 100%;
  border-radius: 5px;
  text-transform: "capitalize";
`;i.a=A=>{const[i,t]=Object(e.useState)();return Object(e.useEffect)((()=>{i||c.c.doc(A.module).get().then((A=>{t(A.data())}))}),[]),Object(g.jsx)(E,{onClick:A.onClick,children:i?Object(g.jsxs)(g.Fragment,{children:[Object(g.jsx)(n,{"data-test":"module-title",children:i.title}),Object(g.jsx)(s,{children:i.description})]}):Object(g.jsx)(g.Fragment,{children:"Loading..."})})}},83:function(A,i,t){"use strict";var e=t(64),o=t(5);const a=e.a.div`
  display: flex;
  width: 100%;
  flex-wrap: no-wrap;
  overflow: auto;
  padding: 20px;
  height: 400px;
`;i.a=({moduleCards:A})=>Object(o.jsx)(a,{children:A})}}]);
//# sourceMappingURL=6.c30966b5.chunk.js.map