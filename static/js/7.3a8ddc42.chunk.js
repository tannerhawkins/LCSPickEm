(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[7],{207:function(A,e,t){"use strict";t.r(e);var i=t(2),a=t(36),c=t(70),o=t(69),n=t(64),s=t(25),g=t(20),r=t(23),d=t(11),E=t(65),C=t(18),l=t(74),I=t(73),B=t(5);const p=n.a.div`
  background-color: ${s.a.COLOR.GREEN};
  border-radius: 4px;
  width: 80%;
  height: 90px;
  margin: 8px 0px;
  display: flex;
  align-items: center;
  padding: 10px;
`;var h=({id:A,type:e,handleDrag:t,handleDrop:i})=>Object(B.jsx)(p,{draggable:!0,id:A,onDragOver:A=>A.preventDefault(),onDragStart:t,onDrop:i,children:e});const Q=n.a.div`
  display: flex;
  flex-direction: column;
  margin: 40px 0px;
`;var x=A=>{const e=Object(C.b)(),[t,a]=Object(i.useState)(),c=Object(C.c)(I.g),o=A=>{a(A.currentTarget.id)},n=A=>{const i=c.find((A=>A.id===parseInt(t))),a=c.find((e=>e.id===parseInt(A.currentTarget.id))),o=i.order,n=a.order,s=c.map((e=>(e.id===parseInt(t)&&(e.order=n),e.id===parseInt(A.currentTarget.id)&&(e.order=o),e)));e(Object(l.e)(s))};return Object(B.jsx)(Q,{"data-test":"step-container",children:c.sort(((A,e)=>A.order-e.order)).map((A=>Object(B.jsx)(h,{id:parseInt(A.id),boxColor:A.color,type:A.type,handleDrag:o,handleDrop:n,"data-test":"box"},parseInt(A.id))))})};const u=n.a.div`
  background-color: white;
  border-radius: 4px;
  width: 40%;
  height: 500px;
  margin: 8px 0px;
  display: flex;
  align-items: center;
  position: relative;
  justify-content: center;
  padding: 30px;
`,b=n.a.p`
  top: 10px;
  right: 20px;
  position: absolute;
  :hover {
    cursor: pointer;
  }
`,j=n.a.div`
  height: 100vh;
  width: 100vw;
  display: flex;
  position: fixed;
  z-index: 10000;
  align-items: center;
  justify-content: center;
  top: 0;
  background: rgba(0, 0, 0, 0.6);
`,R=n.a.div`
  width: 100%;
`;var O=A=>Object(B.jsx)(j,{style:A.style,children:Object(B.jsxs)(u,{className:A.className,children:[Object(B.jsx)(b,{onClick:A.onClose,children:"X"}),Object(B.jsx)(R,{children:A.children})]})}),m=t.p+"static/media/text icon.b8202f2f.png",J=t.p+"static/media/quiz icon.0ea0620a.png";const S=Object(n.a)(O)``,k=n.a.p`
  text-align: center;
  font-size: 30px;
`,f=n.a.img`
  height: 90%;
`,U=n.a.div`
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: space-between;
  width: 100%;
`,K=Object(n.a)(E.a)`
  background-color: ${s.a.COLOR.GREEN};
  border-radius: 4px;
  width: 50%;
  height: 90px;
  margin: 8px 0px;
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 10px 30px;
  padding-right: 10%;
  font-size: 50px;
`;var Y=({className:A,onClose:e,style:t})=>{const i=Object(d.g)(),a=(Object(C.c)(I.g),A=>{i.push(`/teacher/create-module/edit-${A}`)});return Object(B.jsxs)(S,{style:t,className:A,onClose:e,children:[Object(B.jsx)(k,{children:"Select Step to Add"}),Object(B.jsxs)(U,{children:[Object(B.jsxs)(K,{onClick:()=>a("text"),"data-test":"add-text",children:[Object(B.jsx)(f,{src:m}),"Text"]}),Object(B.jsxs)(K,{onClick:()=>a("video"),"data-test":"add-video",children:[Object(B.jsx)(f,{src:"data:image/png;base64,"}),"Video"]}),Object(B.jsxs)(K,{onClick:()=>a("quiz"),"data-test":"add-quiz",children:[Object(B.jsx)(f,{src:J}),"Quiz"]})]})]})},v=t(76),G=t(75),D=t(35);const y=n.a.div`
  margin-top: ${s.a.HEADER_HEIGHT};
  margin-left: ${s.a.SIDEBAR_WIDTH};
  padding: 10px 50px;
`,F=n.a.p`
  line-height: 36px;
  font-size: 30px;
  font-weight: 600;
`,w=n.a.p`
  font-size: 20px;
`,V=n.a.div`
  display: flex;
  width: 100%;
  align-items: center;
  justify-content: space-between;
`,N=n.a.div``,M=n.a.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
`,W=n.a.form`
  display: flex;
  flex-direction: column;
  outline: none;
  width: 90%;
  max-width: 600px;
  margin-top: 60px;
`,q=n.a.input`
  border: none;
  border-bottom: 1px solid black;
  height: 60px;
  margin-bottom: 70px;
  font-size: 30px;
  outline: none;
`,L=Object(n.a)(E.a)`
  font-size: 25px;
  height: 70px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.DARK_GREEN};
  &:hover {
    cursor: pointer;
  }
`,H=Object(n.a)(L)``,X=Object(n.a)(E.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  min-width: 120px;
  background-color: ${s.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`,T=n.a.p`
  color: red;
  margin-top: -20px;
`,P=n.a.p`
  font-size: 20px;
  font-weight: 600;
  margin: 0;
`;e.default=()=>{const A=Object(d.g)(),e=Object(C.b)(),[t,n]=Object(i.useState)(!1),[s,E]=Object(i.useState)(),p=Object(C.c)(I.f),h=Object(C.c)(I.g),Q=Object(C.c)(v.a);Object(i.useEffect)((()=>{}),[h]);return Object(B.jsxs)(a.a,{title:"Create a Module",description:"Create a Module",children:[Object(B.jsx)(c.a,{}),Object(B.jsx)(o.a,{}),Object(B.jsx)(Y,{style:{display:t?"flex":"none"},onClose:()=>n(!1)}),Object(B.jsxs)(y,{children:[Object(B.jsx)(F,{children:"Create New Module"}),Object(B.jsx)(w,{children:"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce ac purus eget lacus pellentesque consequat. Donec commodo tincidunt lorem, c ursus accumsan eros varius vel."}),Object(B.jsxs)(W,{id:"moduleForm",children:[Object(B.jsx)(P,{children:"Module Title"}),Object(B.jsx)(q,{type:"text",id:"title",name:"title",defaultValue:null===p||void 0===p?void 0:p.title,"data-test":"title",required:!0}),Object(B.jsx)(P,{children:"Module Description"}),Object(B.jsx)(q,{type:"text",id:"description",name:"description","data-test":"description",defaultValue:null===p||void 0===p?void 0:p.description}),Object(B.jsxs)(M,{children:[Object(B.jsx)(P,{children:"Steps - Drag to Change Order"}),Object(B.jsx)(H,{onClick:()=>{e(Object(l.f)(document.getElementById("title").value)),e(Object(l.c)(document.getElementById("description").value)),n(!0)},"data-test":"add-step",children:"Add Step"})]}),Object(B.jsx)(N,{children:Object(B.jsx)(x,{steps:h})}),Object(B.jsx)(T,{children:s}),Object(B.jsxs)(V,{children:[Object(B.jsx)(L,{onClick:()=>{E();const t=Object.values(document.forms.moduleForm).filter((A=>A.name)).reduce(((A,e)=>(A[e.name]=e.value,A)),{}),i={...p,title:t.title,description:t.description,creator:g.a.currentUser.uid};if(""===t.title)return void E("Please fill out module title");if(0===h.length)return void E("Please add at least one step");const a=Q.students;g.c.add(i).then((A=>{g.c.doc(A.id).update({mid:A.id}),g.d.doc(g.a.currentUser.uid).update({moduleList:r.a.firestore.FieldValue.arrayUnion({mid:A.id,title:t.title,description:t.description})}).then((()=>{g.b.doc(Q.cid).update({modules:r.a.firestore.FieldValue.arrayUnion(A.id)}).then(g.b.doc(null===Q||void 0===Q?void 0:Q.cid).get().then((A=>e(Object(G.a)(A.data()))))).then((()=>{a.forEach((t=>{g.d.doc(t.uid).get().then((e=>{const i=e.data().classList,a=i.filter((A=>A.cid===Q.cid))[0],c={...a,modules:[...a.modules,A.id]},o=(()=>{for(let A=0;A<i.length;A++)if(i[A].cid===a.cid)return A;return-1})();i.splice(o,1,c),g.d.doc(t.uid).update({classList:i})})),g.d.doc(g.a.currentUser.uid).get().then((A=>e(Object(D.b)(A.data()))))}))}))}))})).then((()=>A.push("teacher/home")))},"data-test":"create",children:"CREATE"}),Object(B.jsx)(X,{onClick:()=>A.push("teacher/home"),children:"BACK TO DASHBOARD"})]})]})]})]})}},65:function(A,e,t){"use strict";var i=t(25);const a=t(64).a.div`
  background-color: ${i.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;e.a=a},66:function(A,e,t){"use strict";e.a=t.p+"static/media/white mtn.bb2904a1.png"},68:function(A,e,t){"use strict";e.a="data:image/png;base64,"},69:function(A,e,t){"use strict";t(2);var i=t(64),a=t(25),c=t(68),o=t(5);const n=i.a.img`
  height: 50%;
  border-radius: 50%;
`;var s=()=>Object(o.jsx)(n,{src:"data:image/png;base64,"}),g=t(71),r=t(18),d=t(11),E=t(22),C=t(72);const l=i.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,I=Object(i.a)(C.a)`
  height: 50%;
`,B=i.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 800px) {
    display: none;
  }
`,p=i.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,h=i.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var Q=({className:A})=>{const e=Object(r.c)(E.c),t=Object(r.c)(E.a),i=Object(d.g)();return Object(o.jsxs)(l,{className:A,onClick:()=>i.push("/profile"),children:[Object(o.jsx)(I,{}),Object(o.jsxs)(B,{children:[Object(o.jsx)(p,{"data-test":"display-name",children:e}),Object(o.jsx)(h,{"data-test":"account-type",children:t})]})]})},x=(t(35),t(21));const u=i.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${a.a.HEADER_HEIGHT};
  left: ${a.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${a.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,b=i.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,j=i.a.div`
  width: 50%;
  height: 100%;
`,R=i.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,O=i.a.img`
  height: 45%;
`,m=i.a.img`
  height: 40%;
`,J=i.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,S=Object(i.a)(Q)``;e.a=()=>{const A=Object(r.b)();return Object(o.jsxs)(u,{id:"header",children:[Object(o.jsx)(j,{}),Object(o.jsxs)(b,{children:[Object(o.jsx)(S,{}),Object(o.jsxs)(R,{children:[Object(o.jsx)(O,{src:c.a}),Object(o.jsx)(m,{src:"data:image/png;base64,"})]}),Object(o.jsx)(s,{}),Object(o.jsx)(J,{onClick:()=>A(Object(x.a)()),src:g.a})]})]})}},70:function(A,e,t){"use strict";t(2);var i=t(64),a=t(25),c=t(66),o=t(11),n=t(5);const s=i.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${a.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${a.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,g=i.a.div`
  background-color: ${a.a.COLOR.GREEN};
  height: ${a.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,r=i.a.img`
  width: 60px;
`,d=i.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;e.a=()=>{const A=Object(o.g)();return Object(n.jsxs)(s,{children:[Object(n.jsx)(g,{children:Object(n.jsx)(r,{onClick:()=>A.push("/home"),src:c.a})}),Object(n.jsx)(d,{src:"data:image/png;base64,"}),Object(n.jsx)(d,{src:"data:image/png;base64,"}),Object(n.jsx)(d,{src:"data:image/png;base64,"})]})}},71:function(A,e,t){"use strict";e.a="data:image/png;base64,"},72:function(A,e,t){"use strict";t(2);var i=t(64),a=t.p+"static/media/default profile icon.591db299.png",c=t(5);const o=i.a.img`
  height: 50%;
  border-radius: 50%;
`;e.a=({className:A})=>Object(c.jsx)(o,{className:A,src:a})},73:function(A,e,t){"use strict";t.d(e,"d",(function(){return o})),t.d(e,"a",(function(){return n})),t.d(e,"f",(function(){return s})),t.d(e,"g",(function(){return g})),t.d(e,"e",(function(){return r})),t.d(e,"b",(function(){return d})),t.d(e,"c",(function(){return E}));var i=t(17),a=t(21);const c=Object(i.a)((()=>a.c.getState()),(A=>A.module)),o=Object(i.a)(c,(A=>A.inModule)),n=Object(i.a)(c,(A=>A.currentStep)),s=Object(i.a)(c,(A=>A.selectedModule)),g=Object(i.a)(s,(A=>null===A||void 0===A?void 0:A.steps)),r=Object(i.a)(g,(A=>0===A.length?0:Math.max(...A.map((A=>A.id)))+1)),d=Object(i.a)(n,g,((A,e)=>e[A].data)),E=Object(i.a)(n,g,((A,e)=>(A+1)/(e.length+1)*100))},74:function(A,e,t){"use strict";t.d(e,"b",(function(){return i})),t.d(e,"d",(function(){return a})),t.d(e,"a",(function(){return c})),t.d(e,"e",(function(){return o})),t.d(e,"f",(function(){return n})),t.d(e,"c",(function(){return s}));const i=A=>({type:"SET_CURRENT_STEP",payload:A}),a=A=>({type:"SET_SELECTED_MODULE",payload:A}),c=A=>({type:"ADD_STEP",payload:A}),o=A=>({type:"SET_STEPS",payload:A}),n=A=>({type:"SET_TITLE",payload:A}),s=A=>({type:"SET_DESCRIPTION",payload:A})},75:function(A,e,t){"use strict";t.d(e,"a",(function(){return i}));t(20);const i=A=>({type:"SET_CURRENT_CLASS",payload:A})},76:function(A,e,t){"use strict";t.d(e,"a",(function(){return o}));var i=t(17),a=t(21);const c=Object(i.a)((()=>a.c.getState()),(A=>A.class)),o=Object(i.a)(c,(A=>A.currentClass))}}]);
//# sourceMappingURL=7.3a8ddc42.chunk.js.map