(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[19],{119:function(A,t,e){"use strict";var a=e(66),o=e(75),r=e(25),i=e(5);const n=a.a.div`
  border-radius: 3px;
  filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25));
  display: flex;
  align-items: center;
  justify-content: space-between;
  flex-direction: column;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`,c=a.a.img`
  width: 80px;
  height: 80px;
  margin-top: 50px;
`,d=a.a.p``,s=a.a.div`
  align-self: flex-end;
  height: 25px;
  width: 100%;
`;t.a=({children:A,className:t,plusIconColor:e,barColor:a,cardColor:l,onClick:p,dataTest:g})=>{const h=l===r.a.COLOR.GREEN?"white":"black";return Object(i.jsxs)(n,{onClick:p,className:t,style:{backgroundColor:l},"data-test":g,children:[Object(i.jsx)(c,{src:"green"===e?o.a:"data:image/png;base64,"}),Object(i.jsx)(d,{style:{color:h},children:A}),Object(i.jsx)(s,{style:{backgroundColor:a}})]})}},154:function(A,t,e){"use strict";e.r(t);e(2);var a=e(72),o=e(71),r=e(66),i=e(101),n=e(36),c=e(119),d=e(25),s=e(16),l=e(38),p=e(21),g=e(84),h=e(11),b=e(85),u=e(89),x=e(86),C=e(68),O=e(19),f=e(5);const j=r.a.div``,B=r.a.div`
  margin-top: ${d.a.HEADER_HEIGHT};
  margin-left: ${d.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,E=r.a.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
`,m=r.a.p`
  color: ${d.a.COLOR.GREEN};
  line-height: 36px;
  font-size: 30px;
  margin-right: 20px;
`,w=r.a.div`
  display: flex;
  align-items: center;
`,L=r.a.p`
  font-size: 18px;
`,R=r.a.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
  max-width: 950px;
`,I=Object(r.a)(c.a)`
  width: 250px;
  height: 228px;
`,v=r.a.p`
  font-size: 36px;
  line-height: 43px;
  font-weight: 500;
`;t.default=()=>{const A=Object(h.g)(),t=Object(s.b)(),e=Object(s.c)(l.a),r=(Object(s.c)(p.b),null===e||void 0===e?void 0:e.modules),c=null===e||void 0===e?void 0:e.apaModules,y=null===r||void 0===r?void 0:r.map((e=>Object(f.jsx)(b.a,{module:e,"data-test":"module-card",onClick:()=>(e=>{O.d.doc(e).get().then((e=>{t(Object(C.d)(e.data())),A.push("/teacher/create-module")}))})(e)},e))),D=null===c||void 0===c?void 0:c.map((A=>Object(f.jsx)(u.a,{module:A,"data-test":"module-card"},A)));return Object(f.jsx)(n.a,{title:"Dashboard",description:"Dashboard",children:Object(f.jsxs)(j,{children:[Object(f.jsx)(a.a,{}),Object(f.jsx)(o.a,{}),Object(f.jsx)(i.a,{}),Object(f.jsxs)(B,{children:[Object(f.jsxs)(E,{children:[Object(f.jsxs)(w,{children:[Object(f.jsx)(m,{children:"Dashboard "}),Object(f.jsx)(g.a,{})]}),e&&Object(f.jsxs)(L,{children:[Object(f.jsx)("b",{children:"Enrollment Code:"})," ",e.cid]})]}),Object(f.jsx)(v,{children:"New"}),Object(f.jsxs)(R,{children:[e&&Object(f.jsx)(I,{plusIconColor:"green",barColor:d.a.COLOR.GREEN,cardColor:d.a.COLOR.TEAL,onClick:()=>A.push("/teacher/add-module"),dataTest:"add-module",children:"Module"}),e&&Object(f.jsx)(I,{plusIconColor:"teal",barColor:d.a.COLOR.TEAL,cardColor:d.a.COLOR.GREEN,onClick:()=>A.push("/teacher/add-students"),dataTest:"add-student",children:"Students"}),Object(f.jsx)(I,{plusIconColor:"green",barColor:d.a.COLOR.GREEN,cardColor:"white",onClick:()=>A.push("/teacher/create-class"),dataTest:"add-class",children:"Class"})]}),Object(f.jsx)(v,{children:"Assigned Modules"}),Object(f.jsx)(x.a,{moduleCards:null===y||void 0===y?void 0:y.concat(D)})]})]})})}},68:function(A,t,e){"use strict";e.d(t,"b",(function(){return a})),e.d(t,"d",(function(){return o})),e.d(t,"e",(function(){return r})),e.d(t,"a",(function(){return i})),e.d(t,"h",(function(){return n})),e.d(t,"f",(function(){return c})),e.d(t,"g",(function(){return d})),e.d(t,"c",(function(){return s}));const a=A=>({type:"SET_CURRENT_STEP",payload:A}),o=A=>({type:"SET_SELECTED_MODULE",payload:A}),r=A=>({type:"SET_SELECTED_STEP",payload:A}),i=A=>({type:"ADD_STEP",payload:A}),n=A=>({type:"UPDATE_STEP",payload:A}),c=A=>({type:"SET_STEPS",payload:A}),d=A=>({type:"SET_TITLE",payload:A}),s=A=>({type:"SET_DESCRIPTION",payload:A})},84:function(A,t,e){"use strict";var a=e(2),o=e(16),r=e(66),i=e(21),n=e(30),c=e(38),d=e(25),s=e(19),l=e(5);const p=r.a.form`
  display: flex;
  flex-flow: column;
  justify-content: flex-start;
`,g=r.a.select`
  height: 100%;
  padding: 0.5rem;
  border: 0px;
  outline: 0px;
  font-size: 25px;
  color: ${d.a.COLOR.GREEN};
  background-color: ${d.a.COLOR.GRAY};
`,h=r.a.option`
  color: ${A=>A.defaultValue?"lightgrey":"black"};
`;t.a=A=>{const t=Object(o.b)(),e=Object(o.c)(i.b),r=Object(o.c)(c.a);Object(a.useEffect)((()=>{}));return e[0]?Object(l.jsx)(p,{action:A.action,"data-test":"dropdown",children:Object(l.jsx)(g,{id:"services","data-test":"class",name:"services",onChange:A=>{const e=Array.from(A.target.children).filter((A=>A.selected)).map((A=>A.dataset.cid))[0];s.c.doc(e).get().then((A=>t(Object(n.a)(A.data()))))},defaultValue:null===r||void 0===r?void 0:r.cid,children:e.map((A=>Object(l.jsx)(h,{value:A.cid,"data-cid":A.cid,classItem:A,children:A.className},A.cid)))})}):Object(l.jsx)(l.Fragment,{})}},85:function(A,t,e){"use strict";var a=e(2),o=e(66),r=e(25),i=e(19),n=e(5);const c=o.a.div`
  background-color: ${r.a.COLOR.DARK_GREEN};
  filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25));
  display: flex;
  flex-direction: column;
  align-items: center;
  text-transform: "capitalize";
  justify-content: center;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
    background: ${r.a.COLOR.LIGHT_GREEN};
    transform: scale(1.02);
  }
  width: 200px;
  height: 300px;
  padding: 10px;
  min-width: 200px;
  margin: 0px 10px;
  border-radius: 0.5em;
  transition: transform 0.45s ease, background 0.45s ease;
`,d=o.a.p`
  color: ${r.a.COLOR.LIGHT_GREEN}
  font-size: 80px !important;
  text-transform: 'capitalize' !important;
  color: #2E3C40;
  font-weight: 600;
  line-height: 1;
  padding-bottom: 10px;
  padding-top: 10px;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid 
  transition: color .45s ease, border .45s ease;
  font-weight: 600;
  line-height: 1;
  padding-bottom: .5em;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid #2E3C40;
`,s=o.a.p`
  font-size: 15px;
  background-color: white;
  padding: 20px 5px;
  width: 85%;
  height: 100%;
  border-radius: 5px;
  text-transform: "capitalize";
`;t.a=A=>{const[t,e]=Object(a.useState)();return Object(a.useEffect)((()=>{t||i.d.doc(A.module).get().then((A=>{e(A.data())}))}),[]),Object(n.jsx)(c,{onClick:A.onClick,children:t?Object(n.jsxs)(n.Fragment,{children:[Object(n.jsx)(d,{"data-test":"module-title",children:t.title}),Object(n.jsx)(s,{children:t.description})]}):Object(n.jsx)(n.Fragment,{children:"Loading..."})})}},86:function(A,t,e){"use strict";var a=e(66),o=e(5);const r=a.a.div`
  display: flex;
  width: 100%;
  flex-wrap: no-wrap;
  overflow: auto;
  padding: 20px;
  height: 400px;
`;t.a=({moduleCards:A})=>Object(o.jsx)(r,{children:A})},89:function(A,t,e){"use strict";var a=e(2),o=e(66),r=e(25),i=e(19),n=e(5);const c=o.a.div`
  background-color: ${r.a.COLOR.PINK};
  filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25));
  display: flex;
  flex-direction: column;
  align-items: center;
  text-transform: "capitalize";
  justify-content: center;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
    background: ${r.a.COLOR.LIGHT_PINK};
    transform: scale(1.02);
  }
  width: 200px;
  height: 300px;
  padding: 10px;
  min-width: 200px;
  margin: 0px 10px;
  border-radius: 0.5em;
  transition: transform 0.45s ease, background 0.45s ease;
`,d=o.a.p`
  color: ${r.a.COLOR.LIGHT_GREEN}
  font-size: 80px !important;
  text-transform: 'capitalize' !important;
  color: #2E3C40;
  font-weight: 600;
  line-height: 1;
  padding-bottom: 10px;
  padding-top: 10px;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid 
  transition: color .45s ease, border .45s ease;
  font-weight: 600;
  line-height: 1;
  padding-bottom: .5em;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid #2E3C40;
`,s=o.a.p`
  font-size: 15px;
  background-color: white;
  padding: 20px 5px;
  width: 85%;
  height: 100%;
  border-radius: 5px;
  text-transform: "capitalize";
`;t.a=A=>{const[t,e]=Object(a.useState)();return Object(a.useEffect)((()=>{t||i.a.doc(A.module).get().then((A=>{e(A.data())}))}),[]),Object(n.jsx)(c,{onClick:A.onClick,children:t?Object(n.jsxs)(n.Fragment,{children:[Object(n.jsx)(d,{"data-test":"module-title",children:t.title}),Object(n.jsx)(s,{children:t.description})]}):Object(n.jsx)(n.Fragment,{children:"Loading..."})})}}}]);
//# sourceMappingURL=19.a5ad660c.chunk.js.map