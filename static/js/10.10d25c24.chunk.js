(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[10],{56:function(A,e,t){"use strict";t.d(e,"c",(function(){return r})),t.d(e,"b",(function(){return n})),t.d(e,"a",(function(){return a}));var c=t(59);t(60);c.a.initializeApp({apiKey:"AIzaSyBikWf9vy86sF9wXyRjkpdrrAJaw7WoQ8M",authDomain:"ancient-path-adventures.firebaseapp.com",projectId:"ancient-path-adventures",storageBucket:"ancient-path-adventures.appspot.com",messagingSenderId:"1051282870332",appId:"1:1051282870332:web:8f7a5922db7d370e86b339",measurementId:"G-NPPQJQTEQN"});const s=c.a.firestore();var a=c.a.auth();new c.a.auth.GoogleAuthProvider;const r=s.collection("userData"),n=s.collection("classData")},58:function(A,e,t){"use strict";t.d(e,"a",(function(){return c}));const c=A=>({type:"SET_CURRENT_CLASS",payload:A})},86:function(A,e,t){"use strict";t.r(e);var c=t(0),s=t(66),a=t(65),r=t(47),n=t(76),o=t(23),i=t(62),l=t(48),d=t(2);const g=r.a.div`
  border-radius: 3px;
  filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25));
  display: flex;
  align-items: center;
  justify-content: center;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`,C=r.a.img`
  width: 80px;
  height: 80px;
`,B=r.a.p`
  position: absolute;
  align-self: flex-end;
  justify-self: center;
  margin-bottom: 40px;
`,b=r.a.div`
  position: absolute;
  bottom: 0;
  height: 25px;
  width: 100%;
`;var h=({children:A,className:e,plusIconColor:t,barColor:c,cardColor:s,onClick:a})=>{const r=s===l.a.COLOR.GREEN?"white":"black";return Object(d.jsxs)(g,{onClick:a,className:e,style:{backgroundColor:s},children:[Object(d.jsx)(C,{src:"green"===t?i.a:"data:image/png;base64,"}),Object(d.jsx)(B,{style:{color:r},children:A}),Object(d.jsx)(b,{style:{backgroundColor:c}})]})},p=t(11),O=t(10),j=t(13);const f=Object(O.a)((()=>j.b.getState()),(A=>A.class)),u=Object(O.a)(f,(A=>A.currentClass));var w=t(14),x=t(58),E=t(56);const I=A=>Object(d.jsx)(v,{selected:A.selected,"data-cid":A.value.cid,className:A.className,children:A.value.className}),L=r.a.form`
  display: flex;
  flex-flow: column;
  justify-content: flex-start;
`,R=r.a.select`
  height: 100%;
  padding: 0.5rem;
  border: 0px;
  outline: 0px;
  font-size: 25px;
  color: ${l.a.COLOR.GREEN};
`,v=r.a.option`
  color: ${A=>A.selected?"lightgrey":"black"};
`;var Q=A=>{const e=Object(p.b)();return Object(d.jsx)(L,{action:A.action,children:Object(d.jsx)(R,{id:"services",name:"services",onChange:A=>{const t=Array.from(A.target.children).filter((A=>A.selected)).map((A=>A.dataset.cid))[0];E.b.doc(t).get().then((A=>e(Object(x.a)(A.data()))))},children:A.children})})},D=t(5);const m=r.a.div``,F=r.a.div`
  margin-top: ${l.a.HEADER_HEIGHT};
  margin-left: ${l.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,K=r.a.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
`,P=r.a.p`
  color: ${l.a.COLOR.GREEN};
  line-height: 36px;
  font-size: 30px;
`,X=r.a.div`
  display: flex;
  align-items: center;
`,N=r.a.p`
  font-size: 18px;
`,y=r.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  max-width: 950px;
`,M=Object(r.a)(h)`
  width: 250px;
  height: 228px;
`,k=r.a.p`
  font-size: 36px;
  line-height: 43px;
  font-weight: 500;
`;e.default=()=>{const A=Object(D.g)(),e=Object(p.c)(u),t=Object(p.c)(w.b);return Object(c.useEffect)((()=>{}),[e]),Object(d.jsx)(o.a,{title:"Dashboard",description:"Dashboard",children:Object(d.jsxs)(m,{children:[Object(d.jsx)(s.a,{}),Object(d.jsx)(a.a,{}),Object(d.jsx)(n.a,{}),Object(d.jsxs)(F,{children:[Object(d.jsxs)(K,{children:[Object(d.jsxs)(X,{children:[Object(d.jsx)(P,{children:"Dashboard "}),t[0]&&Object(d.jsx)(Q,{children:t.map((A=>Object(d.jsx)(I,{selected:A.cid===e.cid?"selected":"",value:A,classItem:A})))})]}),Object(d.jsx)(N,{children:e&&Object(d.jsxs)("p",{children:[Object(d.jsx)("b",{children:"Enrollment Code:"})," ",e.enrollmentCode]})})]}),Object(d.jsx)(k,{children:"New"}),Object(d.jsxs)(y,{children:[Object(d.jsx)(M,{plusIconColor:"green",barColor:l.a.COLOR.GREEN,cardColor:l.a.COLOR.TEAL,children:"Module"}),Object(d.jsx)(M,{plusIconColor:"teal",barColor:l.a.COLOR.TEAL,cardColor:l.a.COLOR.GREEN,children:"Students"}),Object(d.jsx)(M,{plusIconColor:"green",barColor:l.a.COLOR.GREEN,cardColor:"white",onClick:()=>A.push("/teacher/create-class"),children:"Class"})]})]})]})})}}}]);
//# sourceMappingURL=10.10d25c24.chunk.js.map