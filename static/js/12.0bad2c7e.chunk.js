(this.webpackJsonpAncientPathAdventures=this.webpackJsonpAncientPathAdventures||[]).push([[12],{157:function(A,i,t){"use strict";t.r(i);var e=t(2),o=t(36),a=t(72),c=t(71),n=t(66),s=t(25),d=t(19),r=t(85),g=t(89),E=t(16),l=t(21),C=t(38),h=t(30),B=t(23),I=t(67),p=t(11),x=t(68),Q=t(86),u=t(5);const m=n.a.div`
  margin-top: ${s.a.HEADER_HEIGHT};
  margin-left: ${s.a.SIDEBAR_WIDTH};
  padding: 0 50px;
`,b=n.a.p`
  font-size: 36px;
  line-height: 43px;
  font-weight: 500;
`,j=n.a.div`
  display: flex;
  align-items: space-around;
  justify-content: space-around;
  margin-bottom: 50px;
  width: 60%;
`,R=Object(n.a)(I.a)`
  font-size: 15px;
  height: 50px;
  padding: 0 5px;
  width: 220px;
  background-color: ${s.a.COLOR.LIGHT_GREEN};
  &:hover {
    cursor: pointer;
  }
`;i.default=()=>{const A=Object(E.b)(),i=Object(p.g)(),t=Object(E.c)(l.h),n=Object(E.c)(C.a),[I,O]=Object(e.useState)([]);Object(e.useEffect)((()=>{0==I.length&&d.a.get().then((A=>{O(A.docs.map((A=>A.data())))}))}),[]);const f=null===I||void 0===I?void 0:I.filter((A=>!(null===n||void 0===n?void 0:n.apaModules.includes(A.mid)))).map((A=>Object(u.jsx)(g.a,{onClick:()=>Y(A.mid,!0),module:A.mid},A.mid))),J=null===t||void 0===t?void 0:t.filter((A=>!(null===n||void 0===n?void 0:n.modules.includes(A.mid)))).map((A=>Object(u.jsx)(r.a,{onClick:()=>Y(A.mid,!1),module:A.mid},A.mid))),Y=(i,t)=>{t?d.c.doc(n.cid).update({apaModules:B.a.firestore.FieldValue.arrayUnion(i)}).then(d.c.doc(n.cid).get().then((A=>{A.data().students.forEach((A=>{d.e.doc(A.uid).get().then((t=>{const e=t.data().classList,o=e.filter((A=>A.cid===n.cid))[0],a={...o,apaModules:[...o.modules,i]},c=(()=>{for(let A=0;A<e.length;A++)if(e[A].cid===o.cid)return A;return-1})();e.splice(c,1,a),d.e.doc(A.uid).update({classList:e})}))}))})).then(d.c.doc(null===n||void 0===n?void 0:n.cid).get().then((i=>A(Object(h.a)(i.data())))))):d.c.doc(n.cid).update({modules:B.a.firestore.FieldValue.arrayUnion(i)}).then(d.c.doc(n.cid).get().then((A=>{A.data().students.forEach((A=>{d.e.doc(A.uid).get().then((t=>{const e=t.data().classList,o=e.filter((A=>A.cid===n.cid))[0],a={...o,modules:[...o.modules,i]},c=(()=>{for(let A=0;A<e.length;A++)if(e[A].cid===o.cid)return A;return-1})();e.splice(c,1,a),d.e.doc(A.uid).update({classList:e})}))}))})).then(d.c.doc(null===n||void 0===n?void 0:n.cid).get().then((i=>A(Object(h.a)(i.data()))))))};return Object(u.jsxs)(o.a,{title:"Add a Module",description:"Add a Module",children:[Object(u.jsx)(a.a,{}),Object(u.jsx)(c.a,{}),Object(u.jsxs)(m,{children:[Object(u.jsx)(b,{children:"Existing Modules"}),Object(u.jsx)(Q.a,{moduleCards:J}),Object(u.jsx)(b,{children:"APA Modules"}),Object(u.jsx)(Q.a,{moduleCards:f}),Object(u.jsxs)(j,{children:[Object(u.jsx)(R,{onClick:()=>{A(Object(x.d)(s.a.EMPTY_MODULE)),i.push("create-module")},"data-test":"create-module",children:"CREATE NEW MODULE"}),Object(u.jsx)(R,{onClick:()=>i.push("home"),"data-test":"back",children:"BACK TO DASHBOARD"})]})]})]})}},67:function(A,i,t){"use strict";var e=t(25);const o=t(66).a.div`
  background-color: ${e.a.COLOR.LIGHT_GREEN};
  box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  color: white;
  font-weight: bold;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
  }
`;i.a=o},68:function(A,i,t){"use strict";t.d(i,"b",(function(){return e})),t.d(i,"d",(function(){return o})),t.d(i,"e",(function(){return a})),t.d(i,"a",(function(){return c})),t.d(i,"h",(function(){return n})),t.d(i,"f",(function(){return s})),t.d(i,"g",(function(){return d})),t.d(i,"c",(function(){return r}));const e=A=>({type:"SET_CURRENT_STEP",payload:A}),o=A=>({type:"SET_SELECTED_MODULE",payload:A}),a=A=>({type:"SET_SELECTED_STEP",payload:A}),c=A=>({type:"ADD_STEP",payload:A}),n=A=>({type:"UPDATE_STEP",payload:A}),s=A=>({type:"SET_STEPS",payload:A}),d=A=>({type:"SET_TITLE",payload:A}),r=A=>({type:"SET_DESCRIPTION",payload:A})},69:function(A,i,t){"use strict";i.a=t.p+"static/media/white mtn.bb2904a1.png"},71:function(A,i,t){"use strict";t(2);var e=t(66),o=t(25),a=t(75),c=t(5);const n=e.a.img`
  height: 50%;
  border-radius: 50%;
`;var s=()=>Object(c.jsx)(n,{src:"data:image/png;base64,"}),d=t(76),r=t(16),g=t(11),E=t(21),l=t(77);const C=e.a.div`
  height: 100%;
  display: flex;
  align-items: center;

  :hover {
    cursor: pointer;
  }
`,h=Object(e.a)(l.a)`
  height: 50%;
`,B=e.a.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  padding-top: 6px;
  padding-left: 15px;

  @media (max-width: 800px) {
    display: none;
  }
`,I=e.a.p`
  font-size: 18px;
  line-height: 18px;
  margin: 0;
  font-weight: 500;
`,p=e.a.p`
  font-size: 15px;
  font-weight: 400;
  margin: 0;
  text-transform: capitalize;
`;var x=({className:A})=>{const i=Object(r.c)(E.c),t=Object(r.c)(E.a),e=Object(g.g)();return Object(c.jsxs)(C,{className:A,onClick:()=>e.push("/profile"),children:[Object(c.jsx)(h,{}),Object(c.jsxs)(B,{children:[Object(c.jsx)(I,{"data-test":"display-name",children:i}),Object(c.jsx)(p,{"data-test":"account-type",children:t})]})]})},Q=(t(37),t(22));const u=e.a.header`
  display: flex;
  background-color: white;
  justify-content: space-between;
  align-items: center;
  height: ${o.a.HEADER_HEIGHT};
  left: ${o.a.SIDEBAR_WIDTH};
  position: absolute;
  top: 0;
  width: calc(100% - ${o.a.SIDEBAR_WIDTH});
  @media (max-width: 500px) {
    display: none;
  }
`,m=e.a.div`
  display: flex;
  align-items: center;
  justify-content: space-around;
  padding-right: 3%;
  width: 50%;
  height: 100%;
  max-width: 600px;
`,b=e.a.div`
  width: 50%;
  height: 100%;
`,j=e.a.div`
  display: flex;
  align-items: center;
  height: 100%;
`,R=e.a.img`
  height: 45%;
`,O=e.a.img`
  height: 40%;
`,f=e.a.img`
  height: 35%;
  padding-left: 10%;
  :hover {
    cursor: pointer;
  }
`,J=Object(e.a)(x)``;i.a=()=>{const A=Object(r.b)();return Object(c.jsxs)(u,{id:"header",children:[Object(c.jsx)(b,{}),Object(c.jsxs)(m,{children:[Object(c.jsx)(J,{}),Object(c.jsxs)(j,{children:[Object(c.jsx)(R,{src:a.a}),Object(c.jsx)(O,{src:"data:image/png;base64,"})]}),Object(c.jsx)(s,{}),Object(c.jsx)(f,{onClick:()=>A(Object(Q.a)()),src:d.a})]})]})}},72:function(A,i,t){"use strict";t(2);var e=t(66),o=t(25),a=t(69),c=t(11),n=t(5);const s=e.a.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: ${o.a.SIDEBAR_WIDTH};
  position: fixed;
  left: 0;
  top: 0;
  background-color: ${o.a.COLOR.DARK_BLUE};
  @media (max-width: 500px) {
    display: none;
  }
`,d=e.a.div`
  background-color: ${o.a.COLOR.GREEN};
  height: ${o.a.SIDEBAR_WIDTH};
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
  :hover {
    cursor: pointer;
  }
`,r=e.a.img`
  width: 60px;
`,g=e.a.img`
  width: 40px;
  margin: 50px auto 0px;
  :hover {
    cursor: pointer;
  }
`;i.a=()=>{const A=Object(c.g)();return Object(n.jsxs)(s,{children:[Object(n.jsx)(d,{children:Object(n.jsx)(r,{onClick:()=>A.push("/home"),src:a.a})}),Object(n.jsx)(g,{src:"data:image/png;base64,"}),Object(n.jsx)(g,{src:"data:image/png;base64,"}),Object(n.jsx)(g,{src:"data:image/png;base64,"})]})}},75:function(A,i,t){"use strict";i.a="data:image/png;base64,"},76:function(A,i,t){"use strict";i.a="data:image/png;base64,"},77:function(A,i,t){"use strict";t(2);var e=t(66),o=t.p+"static/media/default profile icon.591db299.png",a=t(5);const c=e.a.img`
  height: 50%;
  border-radius: 50%;
`;i.a=({className:A})=>Object(a.jsx)(c,{className:A,src:o})},85:function(A,i,t){"use strict";var e=t(2),o=t(66),a=t(25),c=t(19),n=t(5);const s=o.a.div`
  background-color: ${a.a.COLOR.DARK_GREEN};
  filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25));
  display: flex;
  flex-direction: column;
  align-items: center;
  text-transform: "capitalize";
  justify-content: center;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
    background: ${a.a.COLOR.LIGHT_GREEN};
    transform: scale(1.02);
  }
  width: 200px;
  height: 300px;
  padding: 10px;
  min-width: 200px;
  margin: 0px 10px;
  border-radius: 0.5em;
  transition: transform 0.45s ease, background 0.45s ease;
`,d=o.a.p`
  color: ${a.a.COLOR.LIGHT_GREEN}
  font-size: 80px !important;
  text-transform: 'capitalize' !important;
  color: #2E3C40;
  font-weight: 600;
  line-height: 1;
  padding-bottom: 10px;
  padding-top: 10px;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid 
  transition: color .45s ease, border .45s ease;
  font-weight: 600;
  line-height: 1;
  padding-bottom: .5em;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid #2E3C40;
`,r=o.a.p`
  font-size: 15px;
  background-color: white;
  padding: 20px 5px;
  width: 85%;
  height: 100%;
  border-radius: 5px;
  text-transform: "capitalize";
`;i.a=A=>{const[i,t]=Object(e.useState)();return Object(e.useEffect)((()=>{i||c.d.doc(A.module).get().then((A=>{t(A.data())}))}),[]),Object(n.jsx)(s,{onClick:A.onClick,children:i?Object(n.jsxs)(n.Fragment,{children:[Object(n.jsx)(d,{"data-test":"module-title",children:i.title}),Object(n.jsx)(r,{children:i.description})]}):Object(n.jsx)(n.Fragment,{children:"Loading..."})})}},86:function(A,i,t){"use strict";var e=t(66),o=t(5);const a=e.a.div`
  display: flex;
  width: 100%;
  flex-wrap: no-wrap;
  overflow: auto;
  padding: 20px;
  height: 400px;
`;i.a=({moduleCards:A})=>Object(o.jsx)(a,{children:A})},89:function(A,i,t){"use strict";var e=t(2),o=t(66),a=t(25),c=t(19),n=t(5);const s=o.a.div`
  background-color: ${a.a.COLOR.PINK};
  filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, 0.25));
  display: flex;
  flex-direction: column;
  align-items: center;
  text-transform: "capitalize";
  justify-content: center;
  &:hover {
    filter: brightness(110%);
    cursor: pointer;
    background: ${a.a.COLOR.LIGHT_PINK};
    transform: scale(1.02);
  }
  width: 200px;
  height: 300px;
  padding: 10px;
  min-width: 200px;
  margin: 0px 10px;
  border-radius: 0.5em;
  transition: transform 0.45s ease, background 0.45s ease;
`,d=o.a.p`
  color: ${a.a.COLOR.LIGHT_GREEN}
  font-size: 80px !important;
  text-transform: 'capitalize' !important;
  color: #2E3C40;
  font-weight: 600;
  line-height: 1;
  padding-bottom: 10px;
  padding-top: 10px;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid 
  transition: color .45s ease, border .45s ease;
  font-weight: 600;
  line-height: 1;
  padding-bottom: .5em;
  margin: 0 0 0.142857143em;
  border-bottom: 2px solid #2E3C40;
`,r=o.a.p`
  font-size: 15px;
  background-color: white;
  padding: 20px 5px;
  width: 85%;
  height: 100%;
  border-radius: 5px;
  text-transform: "capitalize";
`;i.a=A=>{const[i,t]=Object(e.useState)();return Object(e.useEffect)((()=>{i||c.a.doc(A.module).get().then((A=>{t(A.data())}))}),[]),Object(n.jsx)(s,{onClick:A.onClick,children:i?Object(n.jsxs)(n.Fragment,{children:[Object(n.jsx)(d,{"data-test":"module-title",children:i.title}),Object(n.jsx)(r,{children:i.description})]}):Object(n.jsx)(n.Fragment,{children:"Loading..."})})}}}]);
//# sourceMappingURL=12.0bad2c7e.chunk.js.map